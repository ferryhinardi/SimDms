if not exists (select * from sysobjects where name='gnMstCompanyMapping' and xtype='U')
...
select row_number() over(order by a.InvoiceNo) RowNum    
     ...
     , isnull(e.CampaignFlag, '') CampaignFlag
 ...
 where 1 = 1    
   ...
   and isnull(e.CampaignFlag,'') =
	(case when (@DocPrefix = 'INF' and isnull(@IsFscCampaign, 0) = 1) then e.CampaignFlag else '' end)
...
ALTER procedure [dbo].[usprpt_GnTaxDocPending]
...
--set @CompanyCode='6078401'
--set @BranchCode='607840100'
--set @StartDate='20110301'
--set @FPJDate='20110331'
--set @IsFPJCentral='True'
--set @IsCheckAll='True'
--set @DocNo=''
...
SELECT	BranchCode,'UNIT' AS ProfitCenter, InvoiceNo, convert(varchar,InvoiceDate,112) InvoiceDate, 'Belum Approve' AS Status 
...
insert into @doc_pending
SELECT	BranchCode,'FINANCE' AS ProfitCenter, InvoiceNo, convert(varchar,InvoiceDate,112) InvoiceDate, 'Belum Print' AS Status  
...
insert into @doc_pending
SELECT	BranchCode,'UNIT' AS ProfitCenter, InvoiceNo, convert(varchar,InvoiceDate,112) InvoiceDate, 'Belum Generate Pajak' AS Status 
FROM	OmFakturPajakHdr
WHERE	CompanyCode = @CompanyCode 
        AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
...
EXCEPT
SELECT	BranchCode,'UNIT' AS ProfitCenter, DocNo AS InvoiceNo, convert(varchar,DocDate,112) AS InvoiceDate, 'Belum Generate Pajak' AS Status
FROM	GnGenerateTax
WHERE	CompanyCode = @CompanyCode 
        AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
...
insert into @doc_pending
SELECT	BranchCode,'FINANCE' AS ProfitCenter,  InvoiceNo, convert(varchar,InvoiceDate,112) InvoiceDate, 'Belum Generate Pajak' AS Status 
FROM	ARFakturPajakHdr
WHERE	CompanyCode = @CompanyCode 
        AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
		...
		AND CONVERT(VARCHAR, InvoiceDate, 112) >= @StartDate AND CONVERT(VARCHAR, InvoiceDate, 112) < @FPJDate
EXCEPT
SELECT  BranchCode,'FINANCE' AS ProfitCenter,  DocNo AS InvoiceNo, convert(varchar,DocDate,112) AS InvoiceDate, 'Belum Generate Pajak' AS Status  
FROM	GnGenerateTax
WHERE	CompanyCode = @CompanyCode 
        AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
...
if @IsFPJGab = 1
	insert into @doc_pending
	SELECT	BranchCode,'SPAREPART' AS ProfitCenter, FPGNo, convert(varchar,FPGDate,112) InvoiceDate, 'Belum Generate Pajak' AS Status 
	...
	EXCEPT
	SELECT	BranchCode,'SPAREPART' AS ProfitCenter, DocNo AS InvoiceNo, convert(varchar,DocDate,112) AS InvoiceDate, 'Belum Generate Pajak' AS Status 
	FROM	GnGenerateTax
	WHERE	CompanyCode = @CompanyCode 
			AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
	...
	insert into @doc_pending
	SELECT	BranchCode,'SPAREPART' AS ProfitCenter, InvoiceNo, convert(varchar,InvoiceDate,112), 'Belum Generate Pajak' AS Status 
	FROM	SPTrnSFPJHdr
	WHERE	CompanyCode = @CompanyCode 
            AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
...
insert into @doc_pending
SELECT	BranchCode,'SERVICE' AS ProfitCenter, FPJNO AS  InvoiceNo, convert(varchar,FPJDate,112) AS InvoiceDate, 'Belum Generate Pajak' AS Status 
...
EXCEPT
SELECT	BranchCode,'SERVICE' AS ProfitCenter, DocNo AS InvoiceNo, convert(varchar,DocDate,112) AS InvoiceDate, 'Belum Generate Pajak' AS Status  
FROM	GnGenerateTax
WHERE	CompanyCode = @CompanyCode
        AND ((case when @IsUnion = '0' then BranchCode end)= (case when @IsFPJCentral='True' then BranchCode else @BranchCode end)
				or (case when @IsUnion = '1' then BranchCode end) 
...
begin
	...
	select BranchCode,ProfitCenter,InvoiceNo,convert(datetime,InvoiceDate) InvoiceDate,Status
	from @doc_pending a
	where not exists(
...
ALTER procedure [dbo].[usprpt_GnGenerateSeqTaxWoBranch]
	@CompanyCode as varchar(15)
	,@BranchCode as varchar(15)
...
begin
 ...
 ---------------------------------------------------------------------------------------------------
	...
	--set @CountUnValidFPJ = (select COUNT(TotalUnValid) from
	--						(
	--							select COUNT(FPJSeqNo) TotalUnValid
	--							  from #tblSeqNoFPJ 
	--						  group by CompanyCode, BranchCode, Year, SeqNo, FPJSeqNo 
	--							having LEN(FPJSeqNo) <> 11
	--						)#unValidFPJ)
	...
	begin
		--if @CountUnValidFPJ > 0
		--begin
		--	raiserror('Ada Faktur Pajak Aktif yang tidak sesuai format baru. Silahkan setting No Faktur Pajak terlebih dahulu',16 ,1);
		--	return
		--end
		
		--if @TotalFPJ + isnull((select max(OrderNo) from #f1),0)  > @EndFPJ
		--begin
...
END
GO
if  exists (select * from sysobjects where name='svSDMovement' and xtype='U')
...
BEGIN
	...
	, a.ServiceBookNo, a.ChassisNo, a.BasicModel, a.JobType
	, ((b.OperationHour * b.OperationCost) + ((isnull(c.SupplyQty,0) - isnull(c.ReturnQty,0)) * isnull(c.RetailPrice,0))) as TotalApprove
	from SvTrnService a
	left join svTrnSrvTask b
		on b.CompanyCode = a.CompanyCode
		and b.BranchCode = a.BranchCode
		and b.ServiceNo = a.ServiceNo
		and ISNULL(b.BillType, 'F') = 'F'
	left join svTrnSrvItem c
		on c.CompanyCode = a.CompanyCode
		and c.BranchCode = a.BranchCode
		and c.ServiceNo = a.ServiceNo
		and isnull(c.BillType, 'F') = 'F'
	where a.CompanyCode = @CompanyCode
	and a.BranchCode = @BranchCode
