...
var TaskName = "Download Update-20141024.zip";
var TaskNo = "20141024001";
...
var start = function (cfg, callback) {
	...
	var fileName1 = "JobSvc.js";	
	var fileName2 = "notificationsvc.js";
	
    log("Starting " + TaskName + " for " + CurrentDealerCode);
	
	var dir = path.join(__dirname, "../lib/20141024");
	
	fileName1 = path.join(dir,fileName1);
	fileName2 = path.join(dir,fileName2);
			
			download(config.api().downloadLink + "20141024/JobSvc.js",
				...
				download(config.api().downloadLink + "20141024/notificationsvc.js",
						 fileName2, function(err){
							if (err) log(err);
							
							if (!fs.existsSync(fileName2))
								log( fileName2 + ' update failed');
							else
								log( fileName2 + ' updated successfully');
