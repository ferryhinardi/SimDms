...
BEGIN
	...
	set	@sysDate = '2014/11/01'  --getdate()
	...
	select * into #t1
	 ...
	 select * into pmKDP_SPK_20141103
         ...
         begin try
		   update pmKDP
		      set LastProgress='LOST', 
			      LastUpdateStatus='2014/11/03', --@SysDate, 
				  LostCaseDate='2014/11/03',     --@SysDate, 
				  LostCaseCategory='F', LostCaseReasonID='70', 
			      LostCaseOtherReason='AUTOMATIC LOST FOR SPK OVER 3 MONTHS', 
			      LastUpdateBy='SYSTEM', 
				  LastUpdateDate='2014/11/03'    --@SysDate
	...
	select 'Before: ' BEF, count(*) Total 
	  from (select distinct CompanyCode, BranchCode, InquiryNumber 
	          from pmKDP_SPK_20141103) a
	select 'After : ' AFT, count(*) Total from pmKDP 
	 where LastProgress='LOST' 
	   and LostCaseOtherReason='AUTOMATIC LOST FOR SPK OVER 3 MONTHS' 
	   and LastUpdateDate='2014/11/03' --@sysDate
    ...
    select * from pmStatusHistory s
     where exists (select top 1 1 from pmKDP 
       ...
       and UpdateDate='2014/11/03' --@sysDate
