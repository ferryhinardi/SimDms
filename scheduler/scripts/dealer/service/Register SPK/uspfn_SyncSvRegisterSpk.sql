...
if object_id('uspfn_SyncSvRegisterSpk') is not null
	drop procedure uspfn_SyncSvRegisterSpk

GO
create procedure [dbo].[uspfn_SyncSvRegisterSpk]
...
select a.CompanyCode
	 ...
	 , OperationName = isnull((
	     select top 1 Description from SvMstTask tsk
		  where tsk.CompanyCode = a.CompanyCode
			and tsk.ProductType = a.ProductType
			and tsk.BasicModel = a.BasicModel
			and tsk.OperationNo = b.OperationNo
			and tsk.JobType = (case when a.JobType in ('CLAIM', 'OTHER') then tsk.JobType else a.JobType end)
	   ), '')
	 , b.OperationHour
