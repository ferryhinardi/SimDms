    ...
    public class DefaultController : Controller
    {
        ...
        protected string SalesCode
        {
            get
            {
                ...
                var myclaims = (User as ClaimsPrincipal).Claims;
                var _UserName = myclaims.First(x => x.Type == "sales_code").Value;
                return _UserName;
            }
        }

        protected bool LoginAsSales
        {
            get
            {
                bool ret = true;
                ...
                if (myclaims != null)
                {
                    var userSales = myclaims.First(x => x.Type == "user_sales").Value;
                    ret = userSales == "1" ? true : false;                    
                }
                return ret;
            }
        }
        ...
        protected string UserRoleIdentity
        {
            get
            {
                if (!User.Identity.IsAuthenticated)
                ...
                var myclaims = (User as ClaimsPrincipal).Claims;
                var role = myclaims.First(x => x.Type == "role").Value;
                return role;
            }
        }
        
        protected string GetAutoNo(string code)
        {
            string result = "";

            var ret = SqlQuery("exec sp_generate_autono '" + CompanyId + "','" + code + "'");

            if (ret.Count == 1)
            {
                result = ret.Table().Rows[0][0].ToString();
            }

            return result;
        }
        
        protected string IsAllowAccess(string menuId,string accessName)
        {
            string result = "";

            var ret = SqlQuery("exec sp_user_checkpermision_detail '" + UserId + "','" + menuId + "','"+  accessName + "'");
        ...
        {
            return ListCompany();
            //return db.Database.SqlQuery<ComboList>("select companyid value, name [text] from dbo.Get_Company('" + UserId  + "')").ToList();
        }
        ...
        public SaveResult SqlQuery(string sql)
        {
            ...
            using (var connection = factory.CreateConnection())
            {
                using (var command = factory.CreateCommand())
                {
                    try
                    {
                        string cnstr = System.Configuration.ConfigurationManager.ConnectionStrings["DataAccessContext"].ToString();
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (ex.InnerException != null)
                        {
                            ...
                            {
                                ret.message += Environment.NewLine + ex.InnerException.Message;
                            }
                        }
                    }
                    finally
        ...
        protected string IsPermission(string MenuId)
        {
            return db.IsPermission(MenuId, RoleId);
        }
