...
{
    [Authorize]
    [RoutePrefix("potaindo/qa/currency")]
    [Route("{action=List}")]
    public class currencyController : DefaultController
    {
        private string entityName = "Currency";
        private string menuId = "21680";
        ...
        public JsonResult List(int take, int skip, IEnumerable<Sort> sort, Kendo.DynamicLinq.Filter filter)
        {
            ...
            var list = db.Currencies.Where(x => x.CompanyCode == CompanyId).AsQueryable();
        ...
        public JsonResult GridAdd(Currency data)
        {
            ...
            var isFound = db.Currencies.Find(CompanyId, data.Code);

            if (isFound == null)
            {
                data.Code = GetAutoNo("CURRENCY");
        ...
        public JsonResult GridUpdate(Currency data)
        {
        ...
        public JsonResult GridDelete(Currency data)
        {
