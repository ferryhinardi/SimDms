...
namespace SimDms.DataWarehouse.Controllers.Api
{
    ...
    {
        public JsonResult Query(string area, string comp, DateTime start, DateTime end)
        {
            ...
            var data = ctx.Database.SqlQuery<SfMpDashboard>("exec uspfn_MpDashboard @GroupNo=@p0, @DealerCode=@p1, @Start=@p2, @End=@p3, @Total=@p4", area, comp, start, end, 0).AsQueryable();
        ...
        public JsonResult QueryNew(string area, string CompanyCode, DateTime start, DateTime end, int Total)
        {
            //select @GroupNo = '500', @DealerCode = '6093401', @Start = '7/22/2004', @End = '12/22/2014'
            ...
            ctx.Database.CommandTimeout = 30 * 60000;

            var sql = @"exec uspfn_MpDashboardNew @GroupNo=@p0, @DealerCode=@p1, @Start=@p2, @End=@p3, @Total=@p4, @GroupNoNew=@p5";

            if (Total == 2) // Export XLS
            {
                var message = "";
                ...
                try
                {
                    ...
                    {
                        new SqlParameter("@p0", area),
                        new SqlParameter("@p1", ParamDealerCode),
                        new SqlParameter("@p2", start),
                        new SqlParameter("@p3", end),
                        new SqlParameter("@p4", Total),
                        new SqlParameter("@p5", ParamGroupNoNew)
                    };

                    var model = new ManPowerDashboardModel
                    {
                        AreaCode = area,
                        DealerCode = ParamDealerCode,
                        Start = start,
                        End = end
                    };

                    var result = ctx.MultiResultSetSqlQuery(sql, parameters);
            ...
            {
                object[] parameters = {area,ParamDealerCode,start,end,Total,ParamGroupNoNew};
                var data = ctx.Database.SqlQuery<SfMpDashboard>(sql, parameters).AsQueryable();
                if (fa) ctx.Database.CommandTimeout = to; else ctx.Database.CommandTimeout = null;
        ...
        public JsonResult Total(string area, string comp, DateTime start, DateTime end)
        {
            //select @GroupNo = '500', @DealerCode = '6093401', @Start = '7/22/2004', @End = '12/22/2014'
            ...
            ctx.Database.CommandTimeout = 3600;
            var data = ctx.Database.SqlQuery<SfMpDashboard>("exec uspfn_MpDashboard @GroupNo=@p0, @DealerCode=@p1, @Start=@p2, @End=@p3, @Total=@p4", area, comp, start, end, 1).AsQueryable();
            if (fa) ctx.Database.CommandTimeout = to; else ctx.Database.CommandTimeout = null;
        ...
        public JsonResult Excel(string area,string comp, DateTime start, DateTime end)
        {
            var message = "";

            try
            {
                var query = @"exec uspfn_MpDashboard @GroupNo, @DealerCode, @Start, @End, @Total";

                var parameters = new[]
                {
                    new SqlParameter("@GroupNo", area),
                    new SqlParameter("@DealerCode", comp),
