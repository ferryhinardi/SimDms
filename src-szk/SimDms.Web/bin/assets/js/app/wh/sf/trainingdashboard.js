...
$(document).ready(function () {
                ...
                items: [
                    {
                        ...
                        type: "controls",
                        items: [
                            { name: "Area", type: "select", cls: "span4", opt_text: "-- SELECT ALL --" },
                        ]
                    },
                    {
                        text: "Dealer",
    ...
    function initComboYear() {
        $.ajax({
            async: false,
            type: "POST",
            url: 'wh.api/TrainingDashboard/Years',
            success: function (data) {
    ...
    function initComboMonth() {
        $.ajax({
            async: false,
            type: "POST",
            url: 'wh.api/TrainingDashboard/Months',
            success: function (data) {
                ...
                if (clkNew === clkOld) {
                    ...
                    if (cellValue != '-' && cellValue != '' && e.row != rCount - 1 && e.row > 1 && e.col > 0 && e.col < 19) {
                        ...
                        $.ajax({
                            ...
                            url: 'wh.api/TrainingDashboard/QryToGridNew',
                            success: function (data) {
                                if (data.message == "") {
    ...
    $('#btnLoadData').on('click', function () {
        ...
        $('#wxgrid').empty();
        rCount === 0 ? iHeight = 400 : iHeight = (rCount + 3) * 25;
        renderGrid();
    ...
    $('#btnExcel').on('click', function () {
        ...
        $.ajax({
            ...
            url: 'wh.api/TrainingDashboard/QueryNew',
            success: function (data) {
                if (data.message == "") {
                    location.href = 'wh.api/TrainingDashboard/DownloadExcelFile?key=' + data.value;
                } else {
                    sdms.info(data.message, "Error");
                }
