...
namespace SimDms.Web
{
    public class SqlMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            {
                if (ctx.SysUsers.Where(Usr => Usr.Username == username).Any())
                {
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            using (DataContext Context = new DataContext())
            {
                return Context.SysUsers.Where(Usr => Usr.LastActivityDate > DateActive).Count();
            }
        }
        ...
        public override MembershipUserCollection FindUsersByEmail(string emailToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataContext Context = new DataContext())
            {
                ...
                IQueryable<SysUser> Users = Context.SysUsers.Where(Usr => Usr.Email == emailToMatch).OrderBy(Usrn => Usrn.Username).Skip(pageIndex * pageSize).Take(pageSize);
        ...
        public override MembershipUserCollection FindUsersByName(string usernameToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataContext Context = new DataContext())
            {
                totalRecords = Context.SysUsers.Where(Usr => Usr.Username == usernameToMatch).Count();
                IQueryable<SysUser> Users = Context.SysUsers.Where(Usr => Usr.Username == usernameToMatch).OrderBy(Usrn => Usrn.Username).Skip(pageIndex * pageSize).Take(pageSize);
                foreach (SysUser user in Users)
                {
                    MembershipUsers.Add(new MembershipUser(Membership.Provider.Name, user.Username, user.UserId, user.Email, null, null, user.IsApproved, user.IsLockedOut, user.CreateDate.Value, user.LastLoginDate.Value, user.LastActivityDate.Value, user.LastPasswordChangedDate.Value, user.LastLockoutDate.Value));
                }
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataContext Context = new DataContext())
            {
                ...
                IQueryable<SysUser> Users = Context.SysUsers.OrderBy(Usrn => Usrn.Username).Skip(pageIndex * pageSize).Take(pageSize);
                foreach (SysUser user in Users)
                {
                    MembershipUsers.Add(new MembershipUser(Membership.Provider.Name, user.Username, user.UserId, user.Email, null, null, user.IsApproved, user.IsLockedOut, user.CreateDate.Value, user.LastLoginDate.Value, user.LastActivityDate.Value, user.LastPasswordChangedDate.Value, user.LastLockoutDate.Value));
                }
