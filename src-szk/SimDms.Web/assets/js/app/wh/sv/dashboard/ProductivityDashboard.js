$(document).ready(function () {
    ...
    function panelInitialization() {
        ...
        $('#pnlChart').append('<div id="tableProductivity" style="margin-top: 0; padding-left: 10px;"></div>');
    }
    ...
    function createKPI() {
        ...
        $.ajax({
            url: 'wh.api/URDashboard/ReloadKeyIndicator',
            type: "POST",
            data: params,
            dataType: 'JSON',
            async: true,
    ...
    function createQTable() {
        ...
        $.ajax({
            url: 'wh.api/ProdDashboard/GetStallNational',
            type: "POST",
            data: params,
            dataType: 'JSON',
            async: true,
            success: function (response) {
                ...
                tableProductivity.setDataAtCell(dataTableUnit, null, null, 'loadData');
            }
        }).done(function (data, textStatus, jqXHR) {
        });
        $.ajax({
            url: 'wh.api/ProdDashboard/GetTechnicianNational',
            type: "POST",
            data: params,
            dataType: 'JSON',
            async: true,
            success: function (response) {
                ...
                tableProductivity.setDataAtCell(dataTableRevenue, null, null, 'loadData');
            }
        }).done(function (data, textStatus, jqXHR) {
        });
        $.ajax({
            url: 'wh.api/ProdDashboard/GetTargetNational',
            type: "POST",
            ...
            success: function (response) {
                ...
                tableProductivity.setDataAtCell(dataTableUnitTarget, null, null, 'loadData');
                tableProductivity.setDataAtCell(dataTableRevTarget, null, null, 'loadData');
            }
        }).done(function (data, textStatus, jqXHR) {
        });
        ...
        var container = document.getElementById('tableProductivity'),
            settings = {
                data: data,
                minSpareRows: 1,
                contextMenu: true,
                ...
                mergeCells: [
                    ...
                    { row: 5, col: 4, rowspan: 1, colspan: 3 },
                ],
                columns: [
                    { readOnly: true },
                ...
                afterRender: function (TD, row, col, prop, value, cellProperties) {
                    $("#tableProductivity table tr td").css("font-size", ".775em");
                }
            };
        tableProductivity = new Handsontable(container, settings);
        tableProductivity.render();
    ...
    function jsonToRow(returnObj, startRow) {
        ...
        for (var i = 0; i < returnObj.length; i++) {
            ...
            data.push([startRow + 5, 1, checkMonth((typeof returnObj[i].Jul === 'undefined') ? 0 : returnObj[i].Jul, 7)]);
            data.push([startRow + 5, 2, checkMonth((typeof returnObj[i].Aug === 'undefined') ? 0 : returnObj[i].Aug, 8)]);
            data.push([startRow + 5, 3, checkMonth((typeof returnObj[i].Sep === 'undefined') ? 0 : returnObj[i].Sep, 9)]);
            data.push([startRow + 5, 4, checkMonth((typeof returnObj[i].Oct === 'undefined') ? 0 : returnObj[i].Oct, 10)]);
            data.push([startRow + 5, 5, checkMonth((typeof returnObj[i].Nov === 'undefined') ? 0 : returnObj[i].Nov, 11)]);
            data.push([startRow + 5, 6, checkMonth((typeof returnObj[i].Des === 'undefined') ? 0 : returnObj[i].Des, 12)]);
        }
        return data;
    }
