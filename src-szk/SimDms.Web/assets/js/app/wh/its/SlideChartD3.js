...
widget.render(function () {
    ...
    function drawChart() {
        ...
        var groups = ['ERTIGA', 'OTHER', 'PU FUTURA', 'PU MEGA CARRY', 'WAGON R'];
        var slides = [];

        // Executive Summary Chart
        if (filter.summary) {
            slides.push({ type: 'summary', text: 'Executive Summary Chart', api: 'PmDashboardData', params: { name: 'PmExecutiveSummary2' } });
        }
        ...
        if (filter.average) {
            for (var i = 0; i < charts.length; i++) {
                slides.push({
                    type: 'average', api: 'PmDashboardByDay',
        ...
        if (filter.trend) {
            for (var i = 0; i < groups.length; i++) {
                for (var j = 0; j < charts.length; j++) {
                    slides.push({
                        type: 'trend', api: 'PmDashboardByDay2',
        ...
        if (slides.length == 0) {
            widget.showNotification('Tidak terdapat ada chart yan dipilih...!');
        ...
        function callSlide() {
            ...
            //console.log(chartloop, slides[chartloop]);
        ...
        function loadChart(slide) {
            if (!busy) {
                ...
                widget.xpost('wh.api/inquiry/' + slide.api, (slide.params || {}), function (result) {
                    busy = false;
