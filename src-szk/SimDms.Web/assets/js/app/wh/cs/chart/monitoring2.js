...
widget.render(function () {
    ...
    widget.bind({ name: 'Inquiry', text: '-- SELECT INQUIRY --', data: initial.inquiries, onChange: refreshChart });
    widget.bind({ name: 'Year', text: '-- SELECT YEAR --', data: initial.years, onChange: refreshChart });
    widget.bind({ name: 'Month', text: '-- SELECT MONTH --', data: initial.months, onChange: refreshChart });
...
function ExportXls() {
    ...
    data.DateFrom = moment(periode, 'YYYY-M').format('YYYY-MM-DD');
    data.DateTo = moment(periode, 'YYYY-M').add('months', 1).add('days', -1).format('YYYY-MM-DD');
    var params = {
