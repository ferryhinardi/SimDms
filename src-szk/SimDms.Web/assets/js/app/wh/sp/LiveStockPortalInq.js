var widget = new SimDms.Widget({
    title: 'Live Stock Portal : Inquery Mode (4W)',
    xtype: 'panels',
    toolbars: [
            ...
            items: [
                {
                    ...
                    items: [
                        ...
                        { name: "Qty", text: "1", cls: "span1", type: "spinner" },
                        { text: "", cls: "errorQty error", type: "label" },
                    ]
                },
                {
...
function selectedParts(row) {
    ...
    widget.post("wh.api/combo/LiveStockSubs", { "PartNo": row.PartNo }, function (result) {
        var list = [];
...
widget.render(function () {
    ...
    $('#btnPartNo').on('click', function () {
        sdms.lookup({
            title: 'Daftar Part',
            url: 'wh.api/lookupgrid/LookUpPartNo',
    ...
    widget.enable({ value: false, items: ["Area", "Qty", "btnPartNo", "PartName", "PartNo"] });
    ...
    widget.checked("checkArea2", function (result) {
        if (result) {
            widget.enable({ value: true, items: ["btnPartNo", "PartName", "PartNo", "Qty"] });
            widget.setValue({ name: "PartNo", value: "" });
        }
        else {
            widget.enable({ value: false, items: ["btnPartNo", "PartName", "PartNo", "Qty"] });
            widget.setValue({ name: "PartNo", value: "-- ALL PARTS --" });
...
function refreshGrid() {
    ...
    widget.kgrid({
        url: "wh.api/inquiry/LiveStockPartDetailInq",
        name: "pnlResult",
        params: filter,
        selectable: "row",
        serverBinding: true,
...
function exportToExcel() {
    var url = "wh.api/InquiryProd/LiveStockPartInqProd?";
    var filter = widget.serializeObject('pnlFilter');
    var params = ''
