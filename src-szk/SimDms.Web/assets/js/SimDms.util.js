...
SimDms.Widget.prototype.clearForm = function (formName) {
    if (this.isNullOrEmpty(formName)) {
        ...
        $("input[type='radio']").attr("value", "true");
        ...
        $(".main form[name='" + formName + "'] input[type='text'], .main form textarea, .main form select").val("");
        $.each(this.defaultData, function (key, val) {
            $("#" + key + ", input[name='" + key + "']").val(val);
        });

        $("form[name='" + formName + "'] input[type='radio']").attr("value", "true");
...
SimDms.Widget.prototype.initializeUpload = function () {
    var itemsObject = $("button[binding-type='button-upload']");
    var _this = this;

    $.each(itemsObject, function (key, val) {
        var itemObject = $(this);
...
SimDms.Widget.prototype.initializePopupContainer = function () {
    var popupFrame = $(".popup-frame.outer");
    var overlay = $(".overlay");
...
SimDms.Widget.prototype.disableElements = function (elements) {
    $.each(elements || [], function (key, val) {
        $('[name="' + (val || '') + '"]').attr('disabled', true);
    });
}

SimDms.Widget.prototype.enableElements = function (elements) {
    $.each(elements || [], function (key, val) {
