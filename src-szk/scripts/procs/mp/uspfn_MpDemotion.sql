...
go
if object_id('uspfn_MpDemotion') is not null	
	drop procedure uspfn_MpDemotion

go
create procedure uspfn_MpDemotion
		 ...
		 , BranchManagerToSalesHead = (
				select count(x.EmployeeID)
				 ...
				 where x.CompanyCode = a.CompanyCode
				   ...
				   and x.PreviousPosition = 'BM'
		   )
		 , SalesHeadToSalesCoordinator = (
				select count(x.EmployeeID)
				  from #t_employee_fix x
				 where x.CompanyCode = a.CompanyCode
				   and x.BranchCode = a.BranchCode
				   ...
				   and x.PreviousPosition = 'SH'
		   )
		 , SalesCoordinatorToSalesPerson = (
				select count(x.EmployeeID)
				  from #t_employee_fix x
				 where x.CompanyCode = a.CompanyCode
				   and x.BranchCode = a.BranchCode
...
go
exec uspfn_MpDemotion '6006406', '', '2012-01-01', '2014-06-01'
