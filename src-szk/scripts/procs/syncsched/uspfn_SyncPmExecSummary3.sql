ALTER procedure [dbo].[uspfn_SyncPmExecSummary3]
as
...
select * 
    ...
    --                      then isnull(m.GroupModel,' ') else 'OTHERS' end 
        select 'C3' as GroupType, '1. INDENT (M)' Description, 
            ...
            left join SuzukiR4..MsMstGroupModel m
                    on m.ModelType    =i.TipeKendaraan
            where @M0 = (select top 1 left(convert(varchar,isnull(UpdateDate,'1900/01/01'),112),6)
                           from suzukir4..pmStatusHistory
                          where CompanyCode=i.CompanyCode
                            and BranchCode=i.BranchCode
                            and InquiryNumber=i.InquiryNumber
                            and LastProgress = 'SPK'
                          order by UpdateDate desc)
              and not exists (select 1 from SuzukiR4..pmStatusHistory
                            where CompanyCode=i.CompanyCode
                                and BranchCode=i.BranchCode
                                and InquiryNumber=i.InquiryNumber
                                and LastProgress in ('DO','DELIVERY','LOST'))
    ...
    UNION ALL            
        select 'C2' as GroupType, '2. INDENT (M-1)' Description, 
            ...
            left join SuzukiR4..MsMstGroupModel m
                    on m.ModelType    =i.TipeKendaraan
            where @M1 = (select top 1 left(convert(varchar,isnull(UpdateDate,'1900/01/01'),112),6)
                           from suzukir4..pmStatusHistory
                          where CompanyCode=i.CompanyCode
                            and BranchCode=i.BranchCode
                            and InquiryNumber=i.InquiryNumber
    ...
    UNION ALL            
        select 'C1' as GroupType, '3. INDENT (M-2)' Description, 
            ...
            left join SuzukiR4..MsMstGroupModel m
                    on m.ModelType    =i.TipeKendaraan
            where @M2 >= (select top 1 left(convert(varchar,isnull(UpdateDate,'1900/01/01'),112),6)
                           from suzukir4..pmStatusHistory
                          where CompanyCode=i.CompanyCode
                            and BranchCode=i.BranchCode
                            and InquiryNumber=i.InquiryNumber
...
update PmDashboardData 
   set DataCount=(select a.Total from @t_dash a, @t_group b
                   where PmDashboardData.GroupType=a.GroupType
					 and PmDashboardData.GroupSeq =b.GroupSeq
					 and PmDashboardData.GroupCode=a.Model
					 and PmDashboardData.GroupName=a.Model)
 where exists (select top 1 1 from @t_dash a, @t_group b
                where PmDashboardData.GroupType=a.GroupType
				  and PmDashboardData.GroupSeq =b.GroupSeq
				  and PmDashboardData.GroupCode=a.Model
				  and PmDashboardData.GroupName=a.Model)
