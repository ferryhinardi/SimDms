...
namespace eXpressAPI
{
    ...
    public class MyGlobalVar
    {
        ...
        public static string DataSourceDB = System.Configuration.ConfigurationManager.ConnectionStrings["ExternalDB"].ToString();
        public static string DataSourceAspId = System.Configuration.ConfigurationManager.ConnectionStrings["AspId"].ToString();

        public static SaveResult SqlQueryDB(string sql)
        {
        ...
        public static SaveResult SqlQueryUDB(string sql)
        {
            SaveResult ret = new SaveResult(0, false);
            DbProviderFactory factory = DbProviderFactories.GetFactory("System.Data.SqlClient");
            using (var connection = factory.CreateConnection())
            {
                using (var command = factory.CreateCommand())
                {
                    try
                    {
                        connection.ConnectionString = DataSourceAspId;
                        command.Connection = connection;
                        command.CommandTimeout = 3600;
                        command.CommandText = sql;
                        connection.Open();
