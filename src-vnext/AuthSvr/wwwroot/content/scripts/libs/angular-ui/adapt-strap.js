...
angular.module('adaptv.adaptStrap.treebrowser', []).directive('adTreeBrowser', [
  '$adConfig','$http','$timeout',
  function ($adConfig, $http, $timeout) {
    function controllerFunction($scope, $attrs) {
      ...
      if ($attrs.urlNode !== undefined)
      {
          $http.get($scope.$eval($attrs.urlNode))
            .success(function (data, status, headers, config) {
                $scope.treeRoot = data;
                $timeout(function () {
                    $scope.$parent[$attrs.treeRoot] = $scope.treeRoot;
                },500)
