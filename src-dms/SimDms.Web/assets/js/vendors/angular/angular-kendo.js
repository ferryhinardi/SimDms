...
(function(f, define){
  ...
  function getScope(s)
  {
      var domElt = document.getElementById('currentpage');
      return angular.element(domElt).scope()[s]; 
  }
        ...
        link: function(scope, element, attrs, controllers) {
          ...
          var ngForm = controllers[1];
            ...
            var datePickerConfig = {
                format: "dd-MMM-yyyy",
                parseFormats: ["dd-MMM-yyyy","yyyy-MM-dd", "dd/MM/yyyy", "yyyy/MM/dd"],
                footer: "Currently #: kendo.toString(data,'dd-MM-yyyy')#"
            }  */

          //var myConfig = scope[attrs.config] ;
          
           //var sid = "#" + attrs.id;
           //$(sid).data('kendoDatePicker').setOptions(datePickerConfigTest);
          ...
          timeout(function() {
            ...
            var datePickerConfigTest = scope.$parent[attrs.options === undefined ? "defaultDatePickerOptions" : attrs.options];
            console.log(datePickerConfigTest);
           if (myConfig !== undefined )
           {
             var sid = "#" + attrs.id;
             if ($(sid).data('kendoDatePicker') !== undefined)
             {
                $(sid).data('kendoDatePicker').setOptions(myConfig);
             } 
             else if ($(sid).data('kendoDateTimePicker') !== undefined)
             {
                $(sid).data('kendoDateTimePicker').setOptions(myConfig);
             }            
           } */
            ...
            function setupBindings() {
              ...
              OUT: if (ngModel) {
                ...
                ngModel.$render = function() {
                  widget.value(typeof (ngModel.$viewValue) != "undefined" ? ngModel.$viewValue : null)
              ...
              OUT: if (attrs.kNgModel) {
                ...
                scope.$watch(attrs.kNgModel, function(newValue, oldValue)
                {
                  ...
                  {
                      widget.value(null); return;
                  }    
                   var sid = "#" + attrs.id;
                   var dateValue = String(newValue);
                   if ($(sid).data('kendoDatePicker') !== undefined)
                   {
                      if ( dateValue.indexOf("Date(") > -1)
                      {
                        dateValue = new Date(parseInt(newValue.slice(6, -2))); 
                        setter(scope, new moment(dateValue).format("YYYY-MM-DD HH:mm:ss"));                        
                      } else {
                        dateValue = new Date(moment(newValue));
                      }    
                      $(sid).data('kendoDatePicker').value(dateValue);
                   } 
                   else if ($(sid).data('kendoDateTimePicker') !== undefined)
                   {
                      if ( dateValue.indexOf("Date(") > -1)
                      ...
                      $(sid).data('kendoDateTimePicker').value(dateValue);
                   } else {
...
function GetCurrentSize() {
    ...
    {
      scope.$apply(function() {
          scope.WxWidth = window.innerWidth;
          scope.WxHeight = window.innerHeight;
          typeof scope.OnResize === 'function' && scope.OnResize(window.innerWidth,window.innerHeight);
      });      
    }
...
{
    var domElt = document.getElementById('currentpage');
    return angular.element(domElt).scope(); 
}

function StdClientWidth()
{
...
window.onresize = GetCurrentSize;
*/
