...
function omSalesOrderPOController($scope, $http, $injector) {
    var me = $scope;
    ...
    me.disableHdr = function () {
        var x = 'isC3,isC4,SODate,SalesType,RefferenceNo,RefferenceDate,CustomerCode,btnCustomerCode,TOPCode,btnTOPCode,TOPDays,BillTo,btnBillTo,ShipTo,btnShipTo,ProspectNo,SKPKNo,btnSKPKNo,Salesman,btnSalesman,WareHouseCode,btnWareHouseCode,isLeasing,LeasingCo,GroupPriceCode,Insurance,PaymentType,PrePaymentAmt,PrePaymentDate,PrePaymentBy,btnPrePaymentBy,CommissionBy,CommissionAmt,ContractNo,RequestDate,Remark,SalesCode,Installment,FinalPaymentDate,SalesCoordinator,SalesHead,BranchManager';
        var y = x.split(',', 60);
        var z = y.length;
        for (i = 0; i <= z; i++) {
    ...
    me.disableHdrSts1 = function () {
        var x = 'SODate,SalesType,RefferenceNo,RefferenceDate,CustomerCode,TOPDays,ProspectNo,btnSKPKNo,Salesman,WareHouseCode,GroupPriceCode,SalesCode,Installment,FinalPaymentDate,SalesCoordinator,SalesHead,BranchManager';
        var y = x.split(',', 60);
        var z = y.length;
        for (i = 0; i <= z; i++) {
            $('#' + y[i]).attr('disabled', true);
    ...
    me.enableHdr = function () {
        var x = 'isC3,isC4,SODate,SalesType,CustomerCode,btnCustomerCode,TOPCode,btnTOPCode,TOPDays,BillTo,btnBillTo,ShipTo,btnShipTo,ProspectNo,SKPKNo,btnSKPKNo,Salesman,btnSalesman,WareHouseCode,btnWareHouseCode,isLeasing,GroupPriceCode,Insurance,PaymentType,PrePaymentAmt,PrePaymentDate,PrePaymentBy,btnPrePaymentBy,CommissionBy,CommissionAmt,ContractNo,RequestDate,Remark,SalesCode,Installment,SalesCoordinator,SalesHead,BranchManager';
        var y = x.split(',', 60);
        var z = y.length;
        for (i = 0; i <= z; i++) {
    ...
    me.enableHdrCus = function () {
        var x = 'TOPCode,TOPDays,BillTo,BillName,ShipTo,ShipName,Salesman,SalesmanName,GroupPriceCode,GroupPriceName,Remark,SKPKNo';
        var y = x.split(',', 60);
        var z = y.length;
        for (i = 0; i <= z; i++) {
            $('#' + y[i]).val('');
    ...
    me.PONo = function () {
        if (me.data.CustomerCode == '' || me.data.CustomerCode == undefined) { return MsgBox ("Isi kode pelanggan terlebih dahulu!", MSG_INFO) }

        var lookup = Wx.blookup({
            name: "No PO Browse",
            title: "No PO",
            manager: spSalesManager,
            query: new breeze.EntityQuery().from('LookupPONumber').withParameters({ NOPlg: me.data.CustomerCode }),
            defaultSort: "PONo desc",
            columns: [
                { field: "PONo", title: "Nomor PO", width: 100 },
                { field: "PODate", title: "Tanggal PO", width: 80, template: "#= PODate==null ? '' : moment(PODate).format('DD MMM YYYY') #" },
            ]
        });
        lookup.dblClick(function (data) {
            if (data != null) {
                me.data.RefferenceNo = data.PONo;
                me.data.RefferenceDate = data.PODate;
    ...
    me.Customer = function () {
        var lookup = Wx.blookup({
            ...
            title: "Customer",
            manager: spSalesManager,
            query: new breeze.EntityQuery().from('Select4LookupCustomerPO').withParameters({ ProfitCenterCode: me.datas.ProfitCenterCode }),
            defaultSort: "CustomerCode desc",
            columns: [
    ...
    me.SalesModelCode = function () {
        var lookup = Wx.blookup({
            ...
            title: "Model",
            manager: spSalesManager,
            query: new breeze.EntityQuery().from('Select4ModelSOPO').withParameters({ SONo: me.data.SONo, PONo: me.data.RefferenceNo, NOPlg: me.data.CustomerCode }),
    ...
    me.SalesModelYear = function () {
        ...
        if (me.detail.SalesModelCode == '' || me.detail.SalesModelCode == undefined) { return MsgBox("Sales Model Code harus diisi dulu!", MSG_INFO) }

        var lookup = Wx.blookup({
            name: "SalesModelYearLookup",
            title: "Model year",
            manager: spSalesManager,
            query: new breeze.EntityQuery().from('Select4ModelYearPO').withParameters({ PONo: me.data.RefferenceNo, NOPlg: me.data.CustomerCode, SalesModel: me.detail.SalesModelCode }),
    ...
    me.BrowseColour = function () {
        ...
        var lookup = Wx.blookup({
            ...
            title: "Warna",
            manager: spSalesManager,
            query: new breeze.EntityQuery().from("SelectColour4SOPO").withParameters({ SalesModelCode: me.detail.SalesModelCode, SalesModelYear: me.detail.SalesModelYear, PONo: me.data.RefferenceNo, NOPlg: me.data.CustomerCode }),
        ...
        lookup.dblClick(function (data) {
            if (data != null) {
                ...
                me.detail2.Quantity = data.Quantity;
                me.detail2.MaxQty = data.Quantity
    ...
    $('#Quantity').on('blur', function (e) {
        if (me.detail2.Quantity > me.detail2.MaxQty) {
            MsgBox("Nilai yang dimasukan melebihi maksimal Quantity (Maks: " + me.detail2.MaxQty + ")")
            me.detail2.Quantity = me.detail2.MaxQty;
            me.Apply();
        }
    });
        ...
        on: {
            onSelectChange: function () {
                if (me.gridColourInfo.getSelectedId() !== undefined) {
                    ...
                    $http.post('om.api/SalesOrder/getMaxQty', { SalesModelCode: me.detail.SalesModelCode, SalesModelYear: me.detail.SalesModelYear, PONo: me.data.RefferenceNo, ColourCode: me.detail2.ColourCode, NOPlg: me.data.CustomerCode }).
                    success(function (data, status, headers, config) {
                        if (data.success) {
                            me.detail2.MaxQty = data.data + me.detail2.Quantity;
                        }
                    })
...
$(document).ready(function () {
        ...
        panels: [
            ...
            { 
                ...
                items: [
                   ...
                   { name: "RefferenceDate", model: "data.RefferenceDate", text: "PO Date", cls: "span4", type: 'ng-datepicker', disable: true },
                   { name: "RefferenceNo", model: "data.RefferenceNo", text: "NO PO", cls: "span4", type: "popup", click: "PONo()", required: true, validasi: "required", maxlength: 15 },
                   {
                        text: "TOP",
                        type: "controls",
                        cls: "span4",
                   ...
                   //{ name: "RemarkPO", model:"data.RemarkPO", text: "Remark PO:", readonly: true },
                   {
                        text: "Salesman",
                        type: "controls",
                        cls: "span4",
    ...
    function init() {
        SimDms.Angular("omSalesOrderPOController");
    }
