...
function omTandaTerimaSTNKController($scope, $http, $injector) {
    var me = $scope;
    ...
    me.BPKBOutBy = function () {
        var lookup = Wx.blookup({
            ...
            query: new breeze.EntityQuery.from('STNKOutLookup').withParameters({ Type: me.data.BPKBOutType }),
    ...
    me.ChassisCode = function () {
        var lookup = Wx.blookup({
            name: "ChassisCode4STNK",
            title: "Kode Rangka",
            manager: spSalesManager,
            query: new breeze.EntityQuery.from('ChassisCode4STNK').withParameters({ BPKBOutType: me.data.BPKBOutType, BPKBOutBy: me.data.BPKBOutBy }),
    ...
    me.ChassisNo = function () {
        console.log(me.detail.ChassisCode);
        if (me.detail.ChassisCode != undefined) {
            var lookup = Wx.blookup({
                name: "ChassisNo4STNK",
                title: "No Rangka",
                manager: spSalesManager,
                query: new breeze.EntityQuery.from('ChassisNo4STNK').withParameters({ ChassisCode: me.detail.ChassisCode, BPKBOutType: me.data.BPKBOutType, BPKBOutBy: me.data.BPKBOutBy }),
    ...
    me.approve = function (e, param) {
        $http.post('om.api/TandaTerimaSTNK/Approve', me.data).
    ...
    me.printPreview = function () {
        $http.post('om.api/TandaTerimaSTNK/preprint', me.data)
       .success(function (e) {
           if (e.success) {
    ...
    me.saveData = function (e, param) {
        $http.post('om.api/TandaTerimaSTNK/Save', me.data).
            success(function (data, status, headers, config) {
                if (data.success) {
    ...
    me.AddDetail = function (e, param) {
        $http.post('om.api/TandaTerimaSTNK/SaveDetail', { model: me.data, detailModel: me.detail }).
            success(function (data, status, headers, config) {
                if (data.success) {
                    Wx.Success("Data saved...");
    ...
    me.delete = function () {
        MsgConfirm("Are you sure to delete current record?", function (result) {
            if (result) {
                $http.post('om.api/TandaTerimaSTNK/Delete', { model: me.data }).
                success(function (data, status, headers, config) {
                    if (data.success) {
                        $('#Status').html(data.Status);
                        $('#pnlDetail').hide();
    ...
    me.DeleteDetail = function () {
        MsgConfirm("Are you sure to delete current record?", function (result) {
            if (result) {
                $http.post('om.api/TandaTerimaSTNK/DeleteDetail', { model: me.data, detailModel: me.detail }).
                success(function (data, status, headers, config) {
                    if (data.success) {
                        me.detail = {};
                        me.clearTable(me.griddetail);
    ...
    me.browse = function () {
        ...
        var lookup = Wx.blookup({
            ...
            columns: [
                ...
                { field: "BPKBOutTypeName", title: "Tipe Serah" },
    ...
    me.loadDetail = function (data) {
        $http.post('om.api/TandaTerimaSTNK/DetailSalesSTNK', data)
               .success(function (e) {
                   if (e.success) {
...
$(document).ready(function () {
        ...
        panels: [
            {
                name: "pnlA",
                items: [
                    { name: "DocNo", model: "data.DocNo", text: "No.STNK Terima", cls: "span3", placeHolder: "TTS/YY/XXXXXX", disable: true },
                    {
                        type: "controls",
                        text: "Tgl. STNK Terima",
                        cls: "span5",
                        items: [
    ...
    function init(s) {
        SimDms.Angular("omTandaTerimaSTNKController");
    }
