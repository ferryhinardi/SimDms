...
$(document).ready(function () {
    ...
    function initPosition() {
        $.ajax({
            async: false,
            type: "POST",
            url: 'ab.api/InOutData/Positions',
            success: function (data) {
                ...
                if (clkNew === clkOld) {
                    ...
                    if (cellValue != '-' && e.row != rCount - 1 && e.row > 0 && e.col > 0 && e.col < 7) {
                        ...
                        $.ajax({
                            ...
                            url: 'ab.api/InOutData/QryToGrid',
                            success: function (data) {
                                if (data.message == "") {
    ...
    $('#btnLoadData').on('click', function () {
        ...
        $('#wxgrid').empty();
        rCount === 0 ? iHeight = 400 : rCount === 1 ? iHeight = 150 : rCount * 25;
        renderGrid();
    ...
    $('#btnExcel').on('click', function () {
        ...
        $.ajax({
            ...
            url: 'ab.api/InOutData/Query',
            success: function (data) {
                if (data.message == "") {
                    location.href = 'ab.api/InOutData/DownloadExcelFile?key=' + data.value;
                } else {
                    sdms.info(data.message, "Error");
                }
