...
function svEntrySPKController($scope, $http, $injector) {
    ...
    //me.$watch("data.JobOrderNo", function (a, b) {
    ...
    $('#PoliceRegNo').on('blur', function () {
        ...
        me.data.JobTypeDesc = '';
        me.Apply();
        if (me.data.ServiceStatusDesc == undefined && me.data.ServiceStatusDesc == "NEW") {
            if (me.data.JobOrderNo != "" || me.data.JobOrderNo != null) return;
        }
    ...
    me.gridtaskpart = new webix.ui({
        ...
        columns: [
            ...
            {
                id: "StatusDesc", header: "Status", width: 120, format: function (value) {
    ...
    me.alterUI = function (status) {
        if (status == 'N') {
            ...
            $('#btnCustomerCodeBill, #btnPoliceRegNo, #btnJobType, #btnForemanID, #btnMechanicID').removeAttr('disabled');
            $('#Odometer, #LaborDiscPct, #PartDiscPct, #MaterialDiscPct, #ForemanID, #MechanicID, #EstimateFinishDate').removeAttr('disabled');
            $('[name=IsPPN], [name=IsSparepartClaim], [name=ConfirmChangingPart], [name=InsurancePayFlag]').removeAttr('disabled');
            $("#btnAddDtl").parent().hide();
            $(".dtltp").hide();
            ...
            if (svType == '0' || svType == '1') {
                ...
                if (status == 0) {
                    ...
                    $('#btnOpen').hide();
                    $('#btnProcess').addClass('hide');
                    $('#btnClose, #btnSave, #btnLkpKendaraan, #btnLkpPelanggan, #btnCancelSPK').show();
                    $('#btnPoliceRegNo, #CustomerCodeBill, #btnJobType').prop('disabled', false);
                    $("#btnAddDtl").parent().show();
                    ...
                    // tambahan
                    $('#btnCustomerCodeBill, #btnForemanID, #btnMechanicID').attr('disabled', 'disabled');
                    $('#CustomerCodeBill, #Odometer, #LaborDiscPct, #PartDiscPct, #MaterialDiscPct, #ForemanID, #MechanicID, #EstimateFinishDate').attr('disabled', 'disabled');
                    $('[name=IsPPN], [name=IsSparepartClaim], [name=ConfirmChangingPart], [name=InsurancePayFlag]').attr('disabled', 'disabled');
                    $('wxtaskpart').attr('disabled', 'disabled');
                    $("#btnAddDtl").hide();
                }
                else {
                    $('#btnProcess').addClass('hide');
    ...
    me.populateData = function (result) {
        ...
        //me.data.ColourCode = data.ColourCode == "" ? data.ColourCode : data.ColorCodeDesc;
        ...
        //me.data.EstimationNo = data.EstimationNo;
        //me.data.EstimationDate = data.EstimationDate == null ? moment().format('DD MMM YYYY') : moment(data.EstimationDate).format('DD MMM YYYY');
        //me.data.InvoiceNo = data.InvoiceNo;
        //me.data.BookingNo = data.BookingNo;
        //me.data.BookingDate = data.BookingDate;
        //me.data.ServiceStatusDesc = (svType == '0' ? 'ESTIMATION' : svType == '1' ? 'BOOKING' : data.ServiceStatusDesc);
        ...
        //me.data.ColourCode = data.ColorCodeDesc;
        ...
        me.data.TotalSrvAmt = number_format(data.TotalSrvAmt, 0);
        ...
        //$("[name=IsSparepartClaim]").val(data.IsSparepartClaim);
    ...
    me.populateCustBill = function (data) {
        ...
        me.data.FaxNo = data.FaxNo;
        me.data.ContractNo = data.ContractNo;
        me.data.ContractEndPeriod = data.ContractEndPeriod;
        me.data.ContractStatus = data.ContractStatus;
        me.data.ClubCode = data.ClubCode;
        me.data.ClubEndPeriod = data.ClubEndPeriod;
        me.data.ClubStatusDesc = data.ClubStatusDesc;
    ...
    me.populate = function (data, callback) {
        ...
        if (me.detail.ServiceType == '0') {
            var lookup = Wx.klookup({
                ...
                columns: [
                    ...
                    //{ field: "ColorName", title: "Warna", width: 160 },
                    { field: "PhoneNo", title: "No. Telp", width: 150 },
                    { field: "HPNo", title: "No. HP", width: 150 },
                    { field: "ForemanID", title: "SA", width: 200 },
                    { field: "MechanicID", title: "FM", width: 200 },
                    { field: "Odometer", title: "Odometer(KM)", width: 160, format: "{0:#,##0}", filterable: false },
            ...
            var lookup = Wx.klookup({
                ...
                columns: [
                    { field: "BookingNo", title: "No Booking", width: 160 },
                    {
                        field: "BookingDate", title: "Tgl Booking", width: 130,
                   ...
                   { field: "PoliceRegNo", title: "No Polisi", width: 150 },
                    ...
                    { field: "ChassisNo", title: "Chassis No", width: 160 },
                    { field: "EngineNo", title: "Engine No", width: 160 },
                    { field: "ColorCode", title: "Warna", width: 160 },
                    { field: "PhoneNo", title: "No. Telp", width: 150 },
                    { field: "HPNo", title: "No. HP", width: 150 },
                           ...
                           if (result1.data != '') {
                               MsgConfirm(result1.data, function (result) {
                                   ...
                                   .success(function (result2, status, headers, config) {
                                       ...
                                       if (result2.success) {
                                          if (me.detail.ServiceType == '0') {
                                              me.data.JobOrderNo = result2.EstimationNo;
                                          }
                                          else if (me.detail.ServiceType == '1') {
                                              me.data.JobOrderNo = result2.BookingNo;
                                          } else {
                                              me.data.JobOrderNo = result2.JobOrderNo;
                                          }
                                         ...
                                         $http.post('sv.api/spk/save?bOverrideDisc=' + overrideDisc, param)
                                           .success(function (result2, status, headers, config) {
                                               ...
                                               if (result2.success) {
                                                   ...
                                                   me.populate(result2.data, me.refreshData);
                                                   toastr.success('data saved...');
                                                   $('#ServiceType').attr('disabled', 'disabled');
                                               }
                                           });
    ...
    me.addDetail = function () {
       ...
       .success(function (result, status, headers, config) {
        ...
        $("#TaskPartNo").prop('readonly', false);
        $("#TaskPartNo").removeAttr('readonly');
    }
    ...
    me.saveDetail = function () {
        ...
        var params =
        {
            ...
            //$("[Name=TaskPartSeq]").val().replace(",", ""),// me.data.TaskPartSeq.replace(",", ""),
            ChassisCode: me.data.ChassisCode,
            ChassisNo: me.data.ChassisNo,
            BasicModel: me.data.BasicModel,
            JobType: me.data.JobType,
    ...
    me.PoliceRegistNo = function () {
        ...
        //    columns: [
        //        { field: "VinNo", title: "Vin No", sWidth: "110px" },
        //        { field: "PoliceRegNo", title: "Police RegNo", sWidth: "110px" },
    ...
    me.CustCodeBillLkp = function () {
        var lookup = Wx.blookup({
            ...
            //sortings: [[0, "asc"]],
            columns: [
                { field: "CustomerCode", title: "Customer Code", sWidth: "180px" },
                { field: "CustomerName", title: "Customer Name" },
           ...
           if (result.success == true) {
               ...
               if (result.data.GroupJobType == "CLM") {
                   ...
                   if (result.data.PdiFscSeq <= 0 || (result.data.PdiFscSeq >= 8 && result.data.PdiFscSeq <= 9)) {
                       ...
                       //$('[name=BillType],[name=ItemType]').attr('disabled', 'disabled');
                   }
                   else {
                       me.data.BillType = "C";
                       $('[name=BillType],[name=ItemType]').removeAttr('disabled');
               ...
               if (me.grid.data.length > 0) {
                   me.data.ItemType = "0";
                   $('#TaskPartNo, #OprHourDemandQty,#btnTaskPartNo').removeAttr('disabled');
                   $('#Price, #DiscPct').attr('disabled', 'disabled');
               }
               else {
                   me.data.ItemType = "L";
                   $('#TaskPartNo, #OprHourDemandQty, #Price,#btnTaskPartNo').removeAttr('disabled');
    ...
    me.calculateTotal = function () {
        var params = {
            ...
            Price: $("[Name=OprRetailPrice]").val().replace(",", ""),
    ...
    me.refreshData = function () {
        var data = me.data; //$("#pnlServiceInfo").serializeObject();
    ...
    me.TaskPartNoblur = function () {
        $http.post("sv.api/grid/NoPartOpenblur?partno=" + $('#TaskPartNo').val())
        .success(function (result, status, headers, config) {
            if (result.success) {
                me.data.TaskPartNo = result.data.PartNo;
                me.data.TaskPartDesc = result.data.PartName;
                me.data.QtyAvail = result.data.Available;
                //me.data.PartNo = result.data.Quantity;
                me.data.OprRetailPrice = result.data.RetailPriceInclTax;
                me.data.PriceNet = result.data.Price;
                me.Apply();
            }
            else {
                me.lkuTaskPart();
    ...
    me.initialize = function () {
        ...
        $('#JobOrderNo').on('change', function (e) {
            me.createCookies({
                ServiceType: me.detail.ServiceType,
                JobOrderNo: $(this).val()
            });
        });
        //
    ...
    me.showPelanggan = function () {
        var cust = me.data.CustomerCodeBill;
        Wx.loadForm();
        Wx.showForm({ url: "gn/master/customeradmin", params: cust });
    }
        ...
        panels: [
              ...
              },
            {
                ...
                items: [
                    ...
                    { name: "ServiceStatusDesc", model: "data.ServiceStatusDesc", text: "Status", readonly: true, show: false },
                    { name: "SrvTotalSrvAmt", model: "data.SrvTotalSrvAmt", text: "Total Biaya Perawatan", readonly: true, cls: "number-int", show: false }
                ]
            },
            {
                name: "tabpage1",
            ...
            },
            {
                ...
                items: [
                    ...
                    { name: "Odometer", model: "data.Odometer", text: "KM (Odometer)", cls: "span4 number-int", style: "background-color:#ffcccc;" },
                    {
                        text: "Pelanggan",
                        type: "controls",
                        items: [
            ...
            },
            {
                ...
                title: "Kontrak & Club",
                items: [
                    { name: "ContractNo", model: "data.ContractNo", text: "No Kontrak", placeHolder: "No", cls: "span3", readonly: true },
                    { name: "ContractEndPeriod", model: "data.ContractEndPeriod", text: "Berlaku s/d Tgl", cls: "span3", readonly: true },
                    { name: "ContractStatus", model: "data.ContractStatus", text: "Status", cls: "span2", readonly: true },
                    { name: "ClubCode", model: "data.ClubCode", text: "Club No", placeHolder: "No Club", cls: "span3", readonly: true },
                    { name: "ClubEndPeriod", model: "data.ClubEndPeriod", text: "Berlaku s/d Tgl", placeHolder: "expired", cls: "span3", readonly: true },
                    { name: "ClubStatusDesc", model: "data.ClubStatusDesc", text: "Status", cls: "span2", readonly: true }
                ]
            },
            {
                cls: "tabpage1 CustVehicle",
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        name: "ctlDisc",
                        items: [
                            { name: "LaborDiscPct", model: "data.LaborDiscPct", cls: "span2 number", placeHolder: "Jasa", style: "background-color:#ffcccc;", maxlength: 3 },
                            { name: "PartDiscPct", model: "data.PartDiscPct", placeHolder: "Part", cls: "span2 number", style: "background-color:#ffcccc;", maxlength: 3 },
                            { name: "MaterialDiscPct", model: "data.MaterialDiscPct", placeHolder: "Material", cls: "span2 number", style: "background-color:#ffcccc;", maxlength: 3 }
                        ]
                    },
            ...
            },
            {
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        cls: "span6 dtltp",
                        items: [
                            { name: "TaskPartNo", model: "data.TaskPartNo", cls: "span3", placeHolder: "Kode", type: "popup", click: "lkuTaskPart()", required: true, validasi: "required" },
                    ...
                    { name: "SrvTotalSrvAmt", text: "Total Biaya Perawatan", cls: "span6 indent summary number-int", readonly: true, show: true }
                ]
            },
            {
