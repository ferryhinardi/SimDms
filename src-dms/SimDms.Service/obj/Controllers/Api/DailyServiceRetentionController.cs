...
namespace SimDms.Service.Controllers.Api
{
    public class DailyServiceRetentionController : BaseController
    {
        ...
        public JsonResult PopulateData(DateTime date, string optionType, int interval, int range, bool InclPdi)
        {
            var query = string.Format(@"uspfn_SelectDailyRetention '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}'
                ", CompanyCode, BranchCode, date, optionType, interval, range, InclPdi, CurrentUser.UserId);
