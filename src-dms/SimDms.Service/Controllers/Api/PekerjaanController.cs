...
namespace SimDms.Service.Controllers.Api
{
    public class PekerjaanController : BaseController
    {
        ...
        {
            var pbpr = ctx.LookUpDtls.Find(CompanyCode, "PBPR", "0");
            return Json(new
            {
                ...
                WarrantyTimeDim = "D",
                Pbpr = pbpr == null,
                IsFSCLock = ctx.LookUpDtls.Find(CompanyCode, "SPK_FLAG", "LOCK_FLATE_RATE").ParaValue == "1" ? true : false
            });
        }
        ...
        public JsonResult getData(SvRincianJob model, string groupJobType)
        {
            var IsFSCLock = ctx.LookUpDtls.Find(CompanyCode, "SPK_FLAG", "LOCK_FLATE_RATE").ParaValue == "1" ? true : false;
            ...
            var fsc = ctx.LookUpDtls.Find(CompanyCode, "FLAT_RATE", model.JobType);
            record.OperationHour = (groupJobType == "FSC" && IsFSCLock) ? fsc == null ? decimal.Zero : Convert.ToDecimal(fsc.ParaValue) : record.OperationHour;
