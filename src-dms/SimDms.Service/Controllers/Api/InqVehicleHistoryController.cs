...
//            ", CompanyCode, BranchCode, ProductType, PoliceRegNo, ChassisCode, ChassisNo, BasicMode, JobOrderDate, customerCode, isAllBranch);
        ...
        public JsonResult GetVehicleHistoryLocal(string PoliceRegNo, string NoVin)
        {
            var query = string.Format(@"uspfn_SvInqVehicleHistoryNas '{0}','{1}'", PoliceRegNo, NoVin);
            var sqlstr = ctx.Database.SqlQuery<InqVehicleHistoryNasional>(query).AsQueryable();
            return Json(sqlstr);
        }

        public string GetVehicleHistoryNasional(string PoliceRegNo, string NoVin)
        {
            var simdmsUrl = ctx.SysParameters.Find("simdms_api").ParamValue;

            var client = new RestClient(simdmsUrl);
            // client.Authenticator = new HttpBasicAuthenticator(username, password);

            var request = new RestRequest("api/utilinfo3", Method.POST);
            request.AddParameter("novin", NoVin); // adds to POST or URL querystring based on Method
            request.AddParameter("nopol", PoliceRegNo); // adds to POST or URL querystring based on Method
