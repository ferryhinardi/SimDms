...
namespace SimDms.Service.Controllers.Api
{
    public class MainKSGController : BaseController
    {
        ...
        public JsonResult delDetail(string BatchNo)
        {
            var suser = CurrentUser.UserId;
            using (var ctxx = new SimDms.Service.Models.DataContext(SimDms.Common.MyHelpers.GetConnString("DataContext")))
            {
                if (ctxx.Database.Connection.State == ConnectionState.Closed)
                    ctxx.Database.Connection.Open();
                SqlCommand cmd = ctxx.Database.Connection.CreateCommand() as SqlCommand;
                SqlTransaction trans = ctxx.Database.Connection.BeginTransaction() as SqlTransaction;
