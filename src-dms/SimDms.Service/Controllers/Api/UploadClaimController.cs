    ...
    public class UploadClaimController : BaseController
    {
        ...
        private WarrantyClaimUploadDtls p_WClaimDtlFile(List<WarrantyClaimDtlFile> oWClaimDtlFiles, int i, GnMstCustomer cust)
        {
            var oDtl = new WarrantyClaimUploadDtls()
            {
                ...
                ChassisCodeChassisNo = oWClaimDtlFiles[i].ChassisCodeChasisNo,
                EngineCodeEngineNo = oWClaimDtlFiles[i].EngineCodeEngineNo,
                #endregion
                BasicModel = oWClaimDtlFiles[i].BasicModel,
                ...
                if (ds.Tables.Count > 2)
                {
                    ...
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        var oWCUploadDtls = new WarrantyClaimUploadDtls()
                        {
                            ...
                            ChassisCodeChassisNo = Convert.ToString(row["ChassisCode"]) + Convert.ToString(row["ChassisNo"]),
                            EngineCodeEngineNo = Convert.ToString(row["EngineCode"]) + Convert.ToString(row["EngineNo"]),
                            #endregion
