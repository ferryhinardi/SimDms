...
namespace SimDms.Service.Controllers.Api
{
    public class InvoiceController : BaseController
    {
        ...
        public JsonResult Get(string jobOrderNo, string invoiceNomor)
        {
            ...
            if (invoiceNomor == "")
                ...
                cmd.CommandText = "uspfn_SvTrnInvoiceDraftNew";
            ...
            if (invoiceNomor != "")
                cmd.Parameters.AddWithValue("@InvoiceNo", invoiceNomor ?? "");
        ...
        public JsonResult ProcessInvoice(string jobOrderNo, string remark, string Today)
        {
            companyMD = CompanyMD;
            branchMD = BranchMD;
            ...
            try
            {
                ...
                if (!DateTransValidation(Today)) throw new Exception(err + ", Tanggal transaksi tidak sesuai dengan periode transaksi");
                ...
                if (recordItem.Count() > 0)
                {
                    ...
                    var isIncExist = ctx.Invoices.Where(p => p.CompanyCode == CompanyCode && p.BranchCode == BranchCode && p.ProductType == ProductType && p.JobOrderNo == jobOrderNo).Count() > 0 ? true : false;
                    if (isIncExist)
                    {
                ...
                using (var trans = ctx.Database.BeginTransaction(IsolationLevel.ReadCommitted))
                {
                    try
                    {
                        ...
                        //ctx.Database.CommandTimeout = (int)TimeSpan.FromMinutes(30).TotalSeconds;
                        trans.Commit();
                    }
                    catch (Exception ex)
                    {
                        ...
                        string innerEx = (ex.InnerException == null) ? ex.Message :
                           (ex.InnerException.Message == null) ? (ex.InnerException.InnerException == null) ?
                           ex.Message : ex.InnerException.InnerException.Message : ex.InnerException.Message;
                        var msg = (ex.InnerException == null) ? ex.Message : innerEx;
        ...
        private bool DateTransValidation(string Today)
        {
            ...
            DateTime ltoday = DateTime.Parse(Today).Date;
            ...
            if (oService != null)
            {
                ...
                if (ltoday >= oService.PeriodBeg.Date && ltoday <= oService.PeriodEnd.Date)
                {
                    //if (date <= DateTime.Now.Date)
                    if (ltoday <= stoday)
                    {
                        //if (date >= oService.TransDate.Value.Date)
                        if (ltoday >= oService.TransDate.Value.Date)
                        {
...
";
        ...
        private bool Process(TrnService service, long serviceNo, string remark)
        {
            ...
            try
            {
                ...
                //Jika Campaign, Insert tabel svMstCampaignTrans
                if (service.JobType == "CLAIM")
                {
                    string Qtbl = @"IF NOT EXISTS (SELECT * FROM sys.tables WHERE name LIKE 'svMstCampaignTrans')
                                    CREATE TABLE [dbo].[svMstCampaignTrans](
	                                [CompanyCode] [varchar](15) NOT NULL, [BranchCode] [varchar](15) NOT NULL, [ProductType] [varchar](15) NOT NULL,[ComplainCode] [varchar](15) NOT NULL,[DefectCode] [varchar](15) NOT NULL,
	                                [ChassisCode] [varchar](15) NOT NULL,[ChassisStartNo] [numeric](10, 0) NOT NULL,[ChassisEndNo] [numeric](10, 0) NOT NULL,[OperationNo] [varchar](50) NOT NULL, [Description] [varchar] (150) NOT NULL,
	                                [seq] [smallint] NOT NULL,[JobOrderNo] [varchar](20) NULL,[ClaimDate] [datetime] NULL,[ChassisNo] [numeric](10, 0) NULL,
	                                [CreatedBy] [varchar](15) NOT NULL,[CreatedDate] [datetime] NOT NULL,[LastupdateBy] [varchar](15) NOT NULL,[LastupdateDate] [datetime] NOT NULL,
                                    CONSTRAINT [PK__svMstCampaignTrans] PRIMARY KEY NONCLUSTERED 
                                    ([CompanyCode] ASC,[BranchCode] ASC,[ProductType] ASC,[ComplainCode] ASC,[DefectCode] ASC,
	                                [ChassisCode] ASC,[ChassisStartNo] ASC,[ChassisEndNo] ASC,[OperationNo] ASC, [Description] ASC, [seq] ASC
                                    )WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 70) ON [PRIMARY]
                                    ) ON [PRIMARY]";

                    ctx.Database.ExecuteSqlCommand(Qtbl);
                       
                    var OprNo = ctx.ServiceTasks.Where(i => i.CompanyCode == CompanyCode && i.BranchCode == BranchCode && i.ProductType == service.ProductType && i.ServiceNo == service.ServiceNo).FirstOrDefault().OperationNo;
                    var dcmp = ctx.SvMstCampaigns.Where(x => x.CompanyCode == CompanyCode && x.ChassisCode == service.ChassisCode && service.ChassisNo >= x.ChassisStartNo && service.ChassisNo <= x.ChassisEndNo && x.OperationNo == OprNo && x.IsActive == true).FirstOrDefault();
                    if (dcmp != null)
                    {
                        int seq = 1;
                        var rc = ctx.SvMstCampaignTrans.Where(z => z.CompanyCode == CompanyCode && z.ProductType == dcmp.ProductType && z.ComplainCode == dcmp.ComplainCode && z.DefectCode == dcmp.DefectCode &&
                                        z.ChassisCode == dcmp.ChassisCode && z.ChassisStartNo == dcmp.ChassisStartNo && z.ChassisEndNo == dcmp.ChassisEndNo && z.OperationNo == dcmp.OperationNo).OrderByDescending(o => o.Seq).FirstOrDefault();
                        if (rc == null)
                        {
                            rc = new svMstCampaignTrans
                            {
                                CompanyCode = CompanyCode,
                                BranchCode = BranchCode,
                                ProductType = dcmp.ProductType,
                                ComplainCode = dcmp.ComplainCode,
                                DefectCode = dcmp.DefectCode,
                                ChassisCode = dcmp.ChassisCode,
                                ChassisStartNo = dcmp.ChassisStartNo,
                                ChassisEndNo = dcmp.ChassisEndNo,
                                OperationNo = dcmp.OperationNo,
                                Description = dcmp.Description,
                                Seq = seq,
                                JobOrderNo = service.JobOrderNo,
                                ClaimDate = service.JobOrderDate,
        ...
        private bool Process(string billType, TrnService oService, string remark)
        {
            ...
            switch (billType)
            {
                ...
                case "I":
                    ...
                    //        InvoiceNo = GetNewDocumentNo("INI", DateTime.Now);
                    //}
                    //else InvoiceNo = GetNewDocumentNo("INI", DateTime.Now);
                    //break;
                case "F":
                    if (oService.JobType.Contains("FS") || oService.JobType.Contains("PDI"))
                    {
                    ...
                    else InvoiceNo = GetNewDocumentNo("INF", DateTime.Now);
                    break;                       
                case "W":
            ...
            if (InvoiceNo != "")
            {
                string query = "exec uspfn_SvTrnInvoiceCreateWeb  @p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7";
            ...
            {
                if ((oService.JobType.Contains("FS") || oService.JobType.Contains("PDI")) && oService.IsLocked == true && (billType == "C" || billType == "I"))
                {
                    // Do Nothing
                }
                else
        ...
        private bool UpdateInvoice(Invoice oInvoice, TrnService oService, string billType)
        {
            ...
            oInvoice.LockingBy = myCRC32;
            bool result = false;
            if (oInvoice.IsPKP.Value == true)
            {
                ...
                {
                    if ((oService.JobType.Contains("PDI") || oService.JobType.Contains("FS")) && (billType == "C" || billType == "I"))
                    {
                        if (oService.IsLocked == false)
                            oService.ServiceStatus = "5";
                        else
