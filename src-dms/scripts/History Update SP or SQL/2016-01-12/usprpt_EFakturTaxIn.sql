	...
	SELECT
		  ...
		  FROM (
				 SELECT
					   ...
					   , REPLACE(replace(isnull((case when b.NPWPNo='  .   .   . -   .' then '00.000.000.0-000.000'									-- Perubahan
								 when b.NPWPNo='' then '00.000.000.0-000.000' else b.NPWPNo end),'00.000.000.0-000.000'), '.', ''), '-', '') NPWP   -- Perubahan
				 ...
				 WHERE
					   ...
					   AND (DAY(TaxDate) between DAY(@PeriodFrom) and DAY(@PeriodTo)                             --Penambahan
						OR DAY(convert(varchar,PeriodYear)+'-'+ (case when len(PeriodMonth) = 1 then '0'+ convert(varchar, PeriodMonth) else convert(varchar, PeriodMonth) end)+'-'+ '01') between 
						DAY(DATEADD(dd, -DAY(convert(varchar,PeriodYear)+'-'+ (case when len(PeriodMonth) = 1 then '0'+ convert(varchar, PeriodMonth) else convert(varchar, PeriodMonth) end)+'-'+'01') + 1, convert(varchar,PeriodYear)+'-'+ (case when len(PeriodMonth) = 1 then '0'+ convert(varchar, PeriodMonth) else convert(varchar, PeriodMonth) end)+'-'+ convert(varchar, DAY(@PeriodFrom))))
					   and DAY(DATEADD(dd, -DAY(DATEADD(mm, 1, convert(varchar,PeriodYear)+'-'+ (case when len(PeriodMonth) = 1 then '0'+ convert(varchar, PeriodMonth) else convert(varchar, PeriodMonth) end)+'-01')), DATEADD(mm, 1, convert(varchar,PeriodYear)+'-'+ (case when len(PeriodMonth) = 1 then '0'+ convert(varchar, PeriodMonth) else convert(varchar, PeriodMonth) end)+'-01')))
					   )
					   AND gnTaxIn.IsPKP = 1
	)A
