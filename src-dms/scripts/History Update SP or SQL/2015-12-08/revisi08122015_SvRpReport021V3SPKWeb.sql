...
ALTER PROCEDURE [dbo].[usprpt_SvRpReport021V3SPKWeb]
--DECLARE
	...
	@UserID      varchar(20)
as
--select @CompanyCode=N'6006400001',@BranchCode=N'6006400100',@ProductType=N'4W',@PeriodYear=2015,@Month2=1,@UserID=N'ga'
...
begin
	...
	select a.InvoiceNo, b.OperationHour
	 ...
	 where a.CompanyCode = @CompanyCode
	   ...
	   and Year(a.JobOrderClosed) = @PeriodYear
	   and Month(a.JobOrderClosed) = @Month0
	...
	set @A_15 = isnull((select x.DPPAmt from (select 
					inv.CompanyCode,
					inv.BranchCode,
					year(inv.JobOrderClosed) PeriodYear,
					MONTH(inv.JobOrderClosed) PeriodMonth,
					--sum((invi.SupplyQty * invi.RetailPrice) - (invi.RetailPrice*(invi.discPct/100))) as DPPAmt
					sum((invi.SupplyQty * invi.RetailPrice) - ((invi.RetailPrice*(invi.discPct/100))*(invi.SupplyQty))) as DPPAmt
				from svTrnsrvItem invi	
				inner join [svtrnservice] inv on inv.CompanyCode=invi.CompanyCode and inv.BranchCode=invi.BranchCode and inv.serviceNo=invi.serviceNo
				where invi.companycode=@CompanyCode
				and invi.branchCode=@BranchCode
				--and invi.TypeOfGoods in (2,5,6)
				and invi.TypeOfGoods in ('2','5','6','9')
				and year(inv.JobOrderClosed)=@PeriodYear
				and MONTH(inv.JobOrderClosed)=@Month0
				group by inv.CompanyCode,inv.BranchCode,year(inv.JobOrderClosed),MONTH(inv.JobOrderClosed)
				)x),0)
	...
	set @A_17 = isnull((select sum(isnull(LbrDppAmt, 0)) from @t_inv_tsk where IsSubCon = 1), 0) + isnull((select sum(isnull(SprAmt, 0)) from @t_inv_itm where IsSublet = 1 and  typeofGoods in (3,4,5)), 0)
	--Total Service Sales - Chargeable to customer CPUS (External) (2 + 6 + 11 + 14 + 17)
	set @A_19 = @A_02 + @A_06 + @A_11 + @A_14 + @A_17
	  ...
	  from (
			select a.CompanyCode, a.BasicModel
			 ...
			 where a.CompanyCode = @CompanyCode
			   ...
			   and month(a.JobOrderClosed) = @Month0
			   --and a.JobType != 'REWORK'
			   and a.JobType not in ('REWORK','pdi')
	...
	set @B_02 = (select count(*) from @t_inv_unit)
	set @B_03 = (select count(*) from @t_inv_unit where GroupType = 'P')
	set @B_04 = (select count(*) from @t_inv_unit where GroupType = 'C')
	set @B_05 = (select count(*) from @t_inv_unit_pdi)
	...
	select JobType, ChassisCode, ChassisNo, convert(varchar, JobOrderDate, 112) JobOrderDate
	 ...
	 where 1 = 1
	   and BranchCode = @BranchCode
	   and year(JobOrderClosed) = @PeriodYear
	   and month(JobOrderClosed) = @Month0
	   and JobType != 'REWORK'
	...
	select distinct convert(varchar, a.JobOrderClosed, 112) JobOrderClosed
	 ...
	 where a.CompanyCode = @CompanyCode
	   ...
	   and not exists (
		select 1 from svTrnService
		 where 1 = 1
		   and CompanyCode = a.CompanyCode
		   and BranchCode = a.BranchCode
		   and ChassisCode = a.ChassisCode
		   and ChassisNo = a.ChassisNo
		   and convert(varchar, JobOrderClosed, 112) = convert(varchar, a.JobOrderClosed, 112)
	...
	select a.ChassisCode, a.ChassisNo
	 ...
	 where a.CompanyCode = @CompanyCode
	   and a.BranchCode = @BranchCode
	   and year(a.JobOrderDate) = @PeriodYear
	   and month(a.JobOrderDate) = @Month0
	...
	set @C_12=isnull((select x.sgaInst from (select 
				...
				year(inv.JobOrderClosed) PeriodYear,
				MONTH(inv.JobOrderClosed) PeriodMonth,
				count(invi.ServiceNo) sgaInst
			--from svTrnInvItem invi
			from svTrnSrvItem invi		
			--inner join [svTrnInvoice] inv on inv.CompanyCode=invi.CompanyCode 
			inner join [svTrnService] inv on inv.CompanyCode=invi.CompanyCode
				and inv.BranchCode=invi.BranchCode 
				and inv.ServiceNo=invi.ServiceNo
	...
	--G
	insert into @t_msidat select 'G', 'MSI_089', @Month0, @G_01
	insert into @t_msidat select 'G', 'MSI_090', @Month0, @G_02
	insert into @t_msidat select 'G', 'MSI_091', @Month0, @G_03
	insert into @t_msidat select 'G', 'MSI_092', @Month0, @G_04
	insert into @t_msidat select 'G', 'MSI_093', @Month0, @G_05
	insert into @t_msidat select 'G', 'MSI_094', @Month0, @G_06
...
delete SvHstSzkMSI_SPKV2
 where CompanyCode = @CompanyCode
   and BranchCode = @BranchCode
   and PeriodYear = @PeriodYear
   and PeriodMonth in (select distinct MsiMonth from @t_msidat)

insert into svHstSzkMSI_SPKV2(CompanyCode, BranchCode, PeriodYear, PeriodMonth, SeqNo, MsiGroup, MsiDesc, Unit, MsiData, CreatedBy, CreatedDate)
...
select CompanyCode, BranchCode, PeriodYear, SeqNo, MsiGroup, MsiDesc, Unit
	 ...
	 , isnull([12], 0)  [Month12]
  from (
	select CompanyCode, BranchCode, PeriodYear, PeriodMonth, SeqNo, MsiGroup, MsiDesc, Unit, MsiData from svHstSzkMSI_SPKV2
	 where CompanyCode = @CompanyCode
	   and BranchCode = @BranchCode
	   and PeriodYear = @PeriodYear
