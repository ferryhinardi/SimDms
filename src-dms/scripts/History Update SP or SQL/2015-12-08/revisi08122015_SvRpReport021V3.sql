...
begin
	if exists (
		select * from svHstSzkMSI_Invoice
...
--Total Labour Sales Revenue (2 + 3 + 4)
	...
	select x.*, case y.IsLocked when 0 then 'P' else 'C' end as GroupType 
	  from (
			select a.CompanyCode, a.BasicModel
				 ...
				 --, ((convert(varchar, a.InvoiceDate, 112))+(convert(varchar, a.JobOrderDate, 113))) JobOrderDate
			 ...
			 --group by a.CompanyCode, a.BasicModel, a.ChassisCode, a.ChassisNo, ((convert(varchar, a.InvoiceDate, 112))+(convert(varchar, a.JobOrderDate, 113)))
			)x
	...
	---- FS 1K (1,000 km)
	--set @C_14 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 1000 )
	---- FS 5K (5,000 km)
	--set @C_15 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 5000 )
	---- FS 10K (10,000 km)
	--set @C_16 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 10000 )
	----FS 20K (20,000 km)
	--set @C_17 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 20000 )
	----FS 30K (30,000 km)
	--set @C_18 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 30000)	
	---- FS 40K (40,000 km)
	--set @C_19 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 40000 )
	----FS 50K (50,000 km)
	--set @C_20 = (select count(*) from @t_unit_fsc_clm where GroupJobType = 'FSC' and Odometer = 50000 )
...
exec usprpt_SvRpReport021V3SPK @CompanyCode, @BranchCode, @ProductType, @PeriodYear, @Month1, @Month2, @UserID
