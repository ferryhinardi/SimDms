	...
	BEGIN
		...
		SELECT * INTO #dSls FROM (
			SELECT 
				...
				(SELECT COUNT(StatusProspek) FROM #dByTipe b WHERE b.StatusProspek = '20' AND b.EmployeeID = a.EmployeeID 
				AND b.ModelKendaraan = a.ModelKendaraan) REORDER,
...
Print 'For Print Preview'
	...
	SELECT * INTO #dByTipeP FROM(
		...
		BEGIN
			...
			SELECT PerolehanData SumberData, NEW, REORDER, PROSPECT, HOTPROSPECT, SPK, DO, DELIVERY, LOST FROM #dt37_1P0 ORDER BY PerolehanData
	...
	ELSE
		...
		BEGIN
			...
			SELECT PerolehanData SumberData, NEW, REORDER, PROSPECT, HOTPROSPECT, SPK, DO, DELIVERY, LOST FROM #dt37_1P ORDER BY PerolehanData
	...
	ELSE
		BEGIN
			...
			SELECT PerolehanData SumberData, NEW, REORDER, PROSPECT, HOTPROSPECT, SPK, DO, DELIVERY, LOST FROM #dt37_3P ORDER BY PerolehanData
