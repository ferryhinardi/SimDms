...
BEGIN 
 ...
 -- Posting process : insert data to MD & SD table
		...
		declare @VehicleCOGS			numeric(18,0)
      ...
      --       where sd.CompanyMD        =doc.CompanyCode
             ...
             where left(DocNo,2) in ('IV','IU') 
			   ...
			   and not exists (select top 1 1 from gnMstCompanyMapping 
			                    where gnMstCompanyMapping.CompanyMD=sd.CompanyCode)
...
--	   @curChassisCode ChassisCode, @curChassisNo curChassisNo
				...
				set @VehicleCOGS         = @VehCOGS + @VehCOGSOthers
			  --karena penjualan MD ke SD tidak ada CoGS Others
				set @VehCOGSOthers		 = 0 
					 ...
					 -- MD: Insert data to table omTrSalesSO
						set @sqlString = 'insert into ' +@DBNameMD+ '..omTrSalesSO 
											...
											values(''' +@curCompanyMD+ ''',''' +@curUnitBranchMD+ ''',''' +@SONo+ ''',''' +convert(varchar,@curDocDate,121)+ 
												   ...
												   +@curBranchCode+ ''',NULL,NULL,''POSTING'',''' +@VehWHMD+ ''',0,'' '',''' +@GroupPriceCode+
					 ...
					 -- SD: Insert data to table omTrPurchaseHPP
						...
						set @sqlString = 'insert into ' +@DBName+ '..omTrPurchaseHPP
											...
											values(''' +@curCompanyCode+ ''',''' +@CentralBranch+ ''',''' +@HPPNo+ ''',''' 
												   ...
												   ''',''POSTING'',''' +@currentDate+ ''',0,NULL,''' +convert(varchar,@curDocDate,121)+ ''')' 
												 --''',''POSTING'',''' +@currentDate+ ''',0,NULL,NULL)' 
							execute sp_executesql @sqlString
			 ...
			 -- MD: Insert data to table omTrSalesInvoiceVIN
				set @sqlString = 'insert into ' +@DBNameMD+ '..omTrSalesInvoiceVIN
								  ...
								  values(''' +@curCompanyMD+ ''',''' +@curUnitBranchMD+ ''',''' +@INVNo+ ''',''' +@BPKNo+ ''',''' 
										 ...
										 ''',''' +@curEngineCode+ ''',''' +convert(varchar,@curEngineNo)+ ''',''' 
										 +convert(varchar,@VehicleCOGS)+ ''',0,''' +@curCreatedBy+ ''',''' 
										 +convert(varchar,@curCreatedDate,121)+ ''',''' +@curLastUpdateBy+ ''',''' 
										 +convert(varchar,@curLastUpdateDate,121)+ ''')'
					execute sp_executesql @sqlString
...
select ChassisCode, ChassisNo, SONo, DONo, BPKNo, InvoiceNo from AZSDSMGJKT..omMstVehicle                
 where ChassisCode=@curChassisCode and ChassisNo=@curChassisNo
select * from AZSDSMGJKT..omTrInventTransferOut       where TransferOutNo=@VTONo
select * from AZSDSMGJKT..omTrInventTransferOutDetail where TransferOutNo=@VTONo
select * from AZSDSMGJKT..omTrInventTransferIn        where TransferInNo=@VTINo
select * from AZSDSMGJKT..omTrInventTransferInDetail  where TransferInNo=@VTINo

select * from AZMDIJMGJKT..omHstSDMovement 
 where DocNo in ('SO105/15/000354','DO105/15/000352','SJ105/15/000350','IU105/15/000350')

select CompanyCode, BranchCode, DbName, CompanyMD, BranchMD, UnitBranchMD, DbMD
  from AZMDIJMGJKT..gnMstCompanyMapping
 where CompanyCode='6115204001' and BranchCode='6115204105'

select * from AZMDIJMGJKT..omMstVehicle  where CompanyCode='6115204' and ChassisCode='MH8BG41EAFJ' and ChassisNo=414322

select * from AZMDIJMGJKT..omTrInventTransferOut       where CompanyCode='6115204' and BranchCode='611520401' and TransferOutNo='BTO/11/001089'
select * from AZMDIJMGJKT..omTrInventTransferOutDetail where CompanyCode='6115204' and BranchCode='611520401' and TransferOutNo='BTO/11/001089'
select * from AZMDIJMGJKT..glInterface                 where CompanyCode='6115204' and BranchCode='611520401' and DocNo='BTO/11/001089'
select * from AZMDIJMGJKT..omTrInventTransferIn        where CompanyCode='6115204' and BranchCode='611520405' and TransferInNo='FTI/11/000070'
select * from AZMDIJMGJKT..omTrInventTransferInDetail  where CompanyCode='6115204' and BranchCode='611520405' and TransferInNo='FTI/11/000070'
select * from AZMDIJMGJKT..glInterface                 where CompanyCode='6115204' and BranchCode='611520405' and DocNo='FTI/11/000070'
