...
-- Revision-03 : HTO, 3 Nov 2015 (Fixing SD: COGS Invoice <> HPP)
-- Revision-04 : HTO, 27 Nov 2015 (floor COGS SD)
-- -------------------------------------------------------------------------------
 ...
 -- Posting process : insert data to MD & SD table
        ...
        declare cursvSDMovement cursor for
		  --select *, DocSort = (case when left(DocNo,3)='INV' then 1 else 2 end)
			select *, DocSort = (case when left(DocNo,2) in ('IV','IU') then 1
			                          when left(DocNo,3)='INV'          then 2 
									  else 3 end)
			  from svSDMovement
           --where left(DocNo,3) in ('LMP','INV') 
           --where left(DocNo,2) = 'IN'  -- INV: Invoice Part; INA, INC, INF, INI, INW, INP: Invoice Service 
		   --INV: Invoice Part; 
		   --INA, INC, INF, INI, INW, INP: Invoice Service 
		   --IV & IU: Invoice Unit
             where left(DocNo,2) in ('IN','IV','IU')  
		...
		while (@@FETCH_STATUS =0)
			begin
			 ...
			 -- SD: Purchase Discount
				...
				if left(@curDocNo,2) in ('IV','IU')  --UNIT
				begin
					if @TypeOfGoodsMD='2'
						set @DiscPct=25.00
					else 
				...
				if left(@curDocNo,2) not in ('IV','IU')  --SP&SV
				begin
			  ...
			  --Setup 3 November, karena ada selisih COGS Invoice SD dengan HPP
			  --set @DiscAmt           = round( (@SalesAmt * @DiscPct / 100),0)
			  --Setup 27 November, karena masih ada selisih pembulatan COGS Invoice SD dengan HPP
			  --set @DiscAmt           = @SalesAmt * (@DiscPct * 0.01)
			    set @DiscAmt           = floor(@SalesAmt * (@DiscPct * 0.01))
				set @NetSales          = @SalesAmt - @DiscAmt
			  --Setup 3 November, karena ada selisih COGS Invoice SD dengan HPP
			  --set @PPNAmt            = round((@NetSales*0.1),0)
			  --Setup 27 November, karena masih ada selisih pembulatan COGS Invoice SD dengan HPP
			  --set @PPNAmt            = @NetSales * 0.1
				set @PPNAmt            = floor(@NetSales * 0.1)
			  ...
			  --Setup 27 November, karena masih ada selisih pembulatan COGS Invoice SD dengan HPP
			  --set @RetailPriceNet    = @curRetailPriceMD - (@curRetailPriceMD * (@DiscPct * 0.01))
				set @RetailPriceNet    = @curRetailPriceMD - floor(@curRetailPriceMD * (@DiscPct * 0.01))
