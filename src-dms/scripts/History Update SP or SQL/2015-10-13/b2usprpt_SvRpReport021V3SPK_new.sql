...
begin
	...
	select a.InvoiceNo
		 ...
		 , case when TypeofGoods in ('0','1') then '0'			-- Perubahan 
			when TypeOfGoods in ('2','5','6','7','8') then '2'	-- Perubahan
			else '1' end IsSublet								-- Perubahan
		 , case 
	...
	--Total Lubricant Sales Revenue (11 + 12 + 13)
	set @A_10 = isnull((select sum(SprAmt) from @t_inv_itm where IsSublet = 0 and GroupTpgo = 'OLI'), 0)
	--Lubricant Sales - Chargeable to customer CPUS (External)
	set @A_11 = isnull((select sum(SprAmt) from @t_inv_itm where IsSublet = 0 and GroupTpgo = 'OLI' and GroupJob = 'CPUS'), 0)
	-- Lubricant Sales - Non-chargeable (Warranty, FS 1K - commercial car)
	set @A_12 = isnull((select sum(SprAmt) from @t_inv_itm where IsSublet = 0 and GroupTpgo = 'OLI' and GroupJob = 'CLM,FSC'), 0)
	-- Lubricant Sales - Non Chargeable (Internal, PDI)
	set @A_13 = isnull((select sum(SprAmt) from @t_inv_itm where IsSublet = 0 and GroupTpgo = 'OLI' and GroupJob = 'INT,PDI'), 0)
	...
	set @A_15 = isnull((select x.DPPAmt from (select 
					...
					sum(ceiling((invi.SupplyQty-invi.ReturnQty) * invi.RetailPrice * (100.0 - invi.DiscPct) * 0.01)) as DPPAmt				-- Perubahan
	...
	set @A_16= isnull((select x.DPPAmt from (select 
				...
				where invi.companycode=@CompanyCode
				and invi.branchCode=@BranchCode
				and invi.TypeOfGoods in ('5','6','7','8')
				and year(inv.InvoiceDate)=@PeriodYear
				and MONTH(inv.InvoiceDate)=@Month0
				group by inv.CompanyCode,inv.BranchCode,year(inv.InvoiceDate),MONTH(inv.InvoiceDate)
				)x),0)
	...
	select JobType, ChassisCode, ChassisNo, convert(varchar, JobOrderDate, 112) JobOrderDate
	 ...
	 where 1 = 1
	   and BranchCode = @BranchCode
	   and year(JobOrderDate) = @PeriodYear
	   and month(JobOrderDate) = @Month0
	   and JobType != 'REWORK'
	...
	select distinct convert(varchar, a.JobOrderDate, 112) JobOrderDate
