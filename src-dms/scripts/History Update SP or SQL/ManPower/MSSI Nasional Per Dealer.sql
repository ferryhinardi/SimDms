...
DECLARE curDealer CURSOR LOCAL FAST_FORWARD FOR SELECT GroupNo, DealerCode FROM gnMstDealerMapping WHERE isActive = 1 OPEN curDealer
FETCH NEXT FROM curDealer INTO @GroupNo, @DealerCode
WHILE @@FETCH_STATUS = 0 BEGIN
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		WHILE (@i <= 15) BEGIN
			DECLARE @value INT
			IF(@i <= 12) BEGIN
				(SELECT @value = COUNT(EmployeeID) FROM HrEmployeeTraining WHERE 1 = 1
					AND	CompanyCode = @DealerCode
					AND TrainingCode = @CurrentCode
...
;WITH _1 AS
(
	SELECT a.DealerCode, a.Seq, a.Item, a.LastYear, a.YtdLastYear, a.Ytd, a.Jan, a.Feb, a.Mar, a.Apr, a.May, a.Jun, a.Jul, a.Aug, a.Sep, a.Oct, a.Nov, a.[Dec]
	FROM @ResultTable a
	GROUP BY a.DealerCode, a.Seq, a.Item, a.LastYear, a.YtdLastYear, a.Ytd, a.Jan, a.Feb, a.Mar, a.Apr, a.May, a.Jun, a.Jul, a.Aug, a.Sep, a.Oct, a.Nov, a.[Dec]
), _2 AS
(
	SELECT DISTINCT a.DealerCode
		, DealerAbbr = CASE b.DealerCode WHEN '6006400001' THEN 'SBT' ELSE b.DealerAbbreviation END
		, a.Seq, a.Item, a.LastYear, a.YtdLastYear, a.Ytd, a.Jan, a.Feb, a.Mar, a.Apr, a.May, a.Jun, a.Jul, a.Aug, a.Sep, a.Oct, a.Nov, a.[Dec]
	FROM _1 a
	JOIN gnMstDealerMapping b ON a.DealerCode = b.DealerCode
)
SELECT * FROM _2