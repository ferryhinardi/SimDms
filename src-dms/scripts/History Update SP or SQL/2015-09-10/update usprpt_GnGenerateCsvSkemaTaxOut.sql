...
--select @CompanyCode=N'6083401100',@BranchCodeFrom=N'',@BranchCodeTo=N'',@ProfitCenterCode='200',@PeriodFrom='20150831',
--@PeriodTo='20150831',@ProductType=N'4W', @table='1', 
--@nomorfaktur = '0031539940513' 
...
-- UPDATE NILAI PPN (JIKA IsRecalPPN 0 MAKA HITUNG JUMLAH SUM PPN, JIKA IsRecalPPN 1 MAKA HITUNG PPN = DPP*10%)
IF @IsRecalPPN = 0
BEGIN
SELECT * INTO #t3 FROM
(
SELECT a1.CompanyCode, a1.BranchCode, a1.cBranch, a1.NOMOR_FAKTUR, a1.ProfitCenter, 
(SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode and BranchCode = a1.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, BranchCode, InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN) InvoiceNo,
case when a1.cBranch > 1 then (SELECT TOP 1 KODE_OBJEK FROM @TblTempDetail WHERE CompanyCode = b.CompanyCode and BranchCode = b.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR 
and InvoiceNo = (SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode and BranchCode = a1.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, BranchCode, InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN)) 
ELSE (SELECT TOP 1 KODE_OBJEK FROM @TblTempDetail WHERE CompanyCode = b.CompanyCode and BranchCode = b.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR 
and InvoiceNo = (SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode and BranchCode = a1.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, BranchCode, InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN)) END KodeTempPPN, 
...
INNER join 
	...
	substring(REPLACE(REPLACE(TaxNo, '.', ''), '-', ''),4,13) TaxNo, SUM(PPNAmt) TotalPPNAmt
	from gnTaxOut a
	where 
	a.CompanyCode = @CompanyCode
	and (CASE WHEN @BranchCodeFrom = '' THEN '' ELSE a.BranchCode END) BETWEEN @BranchCodeFrom AND @BranchCodeTo
...
SELECT * INTO #t1 FROM
(
SELECT a1.CompanyCode, --a1.BranchCode, 
a1.cBranch, a1.NOMOR_FAKTUR, a1.ProfitCenter, 
(SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode --and BranchCode = a1.BranchCode 
and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, BranchCode, InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN) InvoiceNo,
case when a1.cBranch > 1 then (SELECT TOP 1 KODE_OBJEK FROM @TblTempDetail WHERE CompanyCode = b.CompanyCode --and BranchCode = b.BranchCode 
and NOMOR_FAKTUR = a1.NOMOR_FAKTUR 
and InvoiceNo = (SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode --and BranchCode = a1.BranchCode 
and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, BranchCode, InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN)) 
ELSE (SELECT TOP 1 KODE_OBJEK FROM @TblTempDetail WHERE CompanyCode = b.CompanyCode --and BranchCode = b.BranchCode 
and NOMOR_FAKTUR = a1.NOMOR_FAKTUR 
and InvoiceNo = (SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode --and BranchCode = a1.BranchCode 
and NOMOR_FAKTUR = a1.NOMOR_FAKTUR GROUP BY CompanyCode, --BranchCode, 
InvoiceNo, KODE_OBJEK, PPN having COUNT(InvoiceNo) = 1 ORDER BY PPN)) END KodeTempPPN, 
case when a1.cBranch > 1 then ((case when b.TotalPpnAmt > a1.JUMLAH_PPN then b.TotalPpnAmt-a1.JUMLAH_PPN else a1.JUMLAH_PPN - b.TotalPpnAmt end)) else (case when b.TotalPpnAmt > a1.JUMLAH_PPN then b.TotalPpnAmt-a1.JUMLAH_PPN else a1.JUMLAH_PPN - b.TotalPpnAmt end) end selisihPPN,
case when a1.cBranch > 1 then ((case when b.TotalPpnAmt > a1.JUMLAH_PPN then '+' else '-' end)) else (case when b.TotalPpnAmt > a1.JUMLAH_PPN then '+' else '-' end) end plusMinPPN
FROM(
SELECT a.CompanyCode, --a.BranchCode, 
(SELECT TOP 1 cBranch FROM @TblTempDetail WHERE CompanyCode = a.CompanyCode AND NOMOR_FAKTUR = a.NOMOR_FAKTUR) cBranch,
NOMOR_FAKTUR, ProfitCenter, SUM(PPN) JUMLAH_PPN
FROM @TblTempDetail a
--WHERE ProfitCenter in ('300','100','200')
GROUP BY a.CompanyCode, --a.BranchCode, 
NOMOR_FAKTUR, ProfitCenter
) a1
INNER join 
	(
	select a.CompanyCode, substring(REPLACE(REPLACE(TaxNo, '.', ''), '-', ''),4,13) TaxNo, FLOOR(SUM(DPPAmt)*0.1) TotalPPNAmt
	from gnTaxOut a
	where 
	a.CompanyCode = @CompanyCode
	and DocumentType = 'F'
	group by a.CompanyCode, substring(REPLACE(REPLACE(TaxNo, '.', ''), '-', ''),4,13)
	) b
		on b.CompanyCode = a1.CompanyCode
		--and b.BranchCode = a1.BranchCode
		and b.TaxNo = a1.NOMOR_FAKTUR
