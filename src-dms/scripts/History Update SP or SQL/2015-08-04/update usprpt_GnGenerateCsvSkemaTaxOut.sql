...
--select @CompanyCode=N'6114201',@BranchCodeFrom=N'',@BranchCodeTo=N'',@PeriodFrom='2015-07-01',@PeriodTo='2015-07-31',@ProductType=N'2W', @table='2'
...
select 
		...
		, CUSTOMERCODE
			from (
						...
						, JUMLAH_DPP = isnull ((convert(decimal(12,2),a.NetSalesAmt)), 0.00)
						, JUMLAH_PPN = isnull ((convert(decimal(12,2),a.NetSalesAmt * 0.10)), 0.00)
						...
						(SELECT Address1 + ' ' + Address2 FROM gnMstCustomer WHERE CompanyCode = b.CompanyCode AND CustomerCode = b.CustomerCode)	
						END END ALAMAT_LENGKAP
						, JUMLAH_DPP = isnull ((convert(decimal(12,0), a.BeforeDiscDPP - a.DiscExcludePPn)), 0)
						, JUMLAH_PPN = isnull ((convert(decimal(12,0),a.AfterDiscPPn)), 0)
						, b.PPNBmAmt JUMLAH_PPNBM 
						, '' ID_KETERANGAN_TAMBAHAN
						, 0 FG_UANG_MUKA
						, 0 UANG_MUKA_DPP
						...
						(SELECT Address1 + ' ' + Address2 FROM gnMstCustomer WHERE CompanyCode = b.CompanyCode AND CustomerCode = b.CustomerCode)	
						END END ALAMAT_LENGKAP
						, JUMLAH_DPP = isnull ((convert(decimal(12,0),a.UnitPriceAmt * a.Quantity)), 0)
						, JUMLAH_PPN = isnull ((convert(decimal(12,0),a.PPNAmt)), 0)
						, b.PPNBmAmt JUMLAH_PPNBM 
						, '' ID_KETERANGAN_TAMBAHAN
						, 0 FG_UANG_MUKA
						, 0 UANG_MUKA_DPP
						...
						(SELECT Address1 + ' ' + Address2 FROM gnMstCustomer WHERE CompanyCode = b.CompanyCode AND CustomerCode = b.CustomerCode)	
						END END ALAMAT_LENGKAP
						, JUMLAH_DPP = isnull (convert(decimal(12,2),(a.RetailPrice * a.SupplyQty)-(a.RetailPrice * a.SupplyQty * DiscPct /100.00),0),0.00)
...
---- tambahan untuk jasa service
						...
						(SELECT Address1 + ' ' + Address2 FROM gnMstCustomer WHERE CompanyCode = b.CompanyCode AND CustomerCode = b.CustomerCode)	
						END END ALAMAT_LENGKAP
						, JUMLAH_DPP = isnull (convert(decimal(12,2),(a.OperationCost * a.OperationHour)-(a.OperationCost * a.OperationHour * DiscPct /100.00),0),0.00) 
...
begin
	select * from (
						...
						, isnull ((convert(decimal(12,0),a.AfterDiscPPn)), 0) PPN
						, 0 TARIF_PPNBM
						, 0 PPNBM
						, b.CUSTOMERCODE 
						...
						select  
							...
							FLOOR(DPP)DPP,
							(FLOOR(DPP)*0.1)PPN,
							--(case when countReff > 1 then floor(DPP) else DPP end) DPP,
							----(case when countReff > 1 then (floor(DPP) * 0.1) else convert(decimal(12,2),(dpp * 0.1)) end) PPN,
							--(case when countReff > 1 then (floor(DPP) * 0.1) else convert(decimal(12,2),(dpp * 0.1)) end) PPN,
							----(floor(DPP) * 0.1) PPN,
							TARIF_PPNBM,
							PPNBM,
