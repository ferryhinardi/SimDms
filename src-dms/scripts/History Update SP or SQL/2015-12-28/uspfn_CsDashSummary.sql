...
update @t_rem set RemValue = (
			...
			from ( select distinct a.CustomerCode, a.CustomerName, a.Chassis, a.PoliceRegNo
				...
				and (b.LockingDate IS NULL OR b.LockingDate = '1900-01-01 00:00:00.000' OR b.LockingDate < b.BPKDate)
...
update @t_rem set RemValue = ( 
		select count(a.CustomerCode)
			...
			where a.CompanyCode like @CompanyCode
			and a.BranchCode like @BranchCode
			and a.LockingDate IS NOT NULL 
			and a.LockingDate <> '1900-01-01 00:00:00.000' 
			and a.LockingDate > a.BPKDate
			and a.LockingDate > (select top 1 SettingParam1 from CSSettings where SettingCode = 'REM3DAYSCALL')
	)
