create procedure [dbo].[uspfn_SvTrnServiceSaveItemWeb]  
--DECLARE
...
if object_id('#tmpSvSDMovement') is not null drop table #tmpSvSDMovement
	  ...
	  -- Untuk ItemPrice Mengambil dari masing-masing dealer	
		set @QueryTemp = 'select   
			...
			from (select
				i.PartNo   
				,i.TypeOfGoods  
				 from ' + dbo.GetDbMD(@CompanyCode, @BranchCode) +'..spMstItems i  
				 where i.CompanyCode = ''' + @CompanyMD + '''  
				 and i.BranchCode  = ''' + @BranchMD + '''  
				 and i.PartNo      = ''' + @PartNo + '''
			) a inner join ' + dbo.GetDbMD(@CompanyCode, @BranchCode) +'..spMstItemPrice b on b.PartNo = a.PartNo
	 ...
	 declare @tblTempPart as table  
	  (  
	   ...
	   TypeOfGoods varchar (2)  
		...
		b.TypeOfGoods 
		 from ' + dbo.GetDbMD(@CompanyCode, @BranchCode) + '..spMstItemPrice a 
		 join ' + dbo.GetDbMD(@CompanyCode, @BranchCode) + '..spMstItems b 
 ...
 begin    
	...
	begin    
		set @QueryTemp = 'update ' + dbo.GetDbMD(@CompanyCode, @BranchCode) + '..svSDMovement set    
 ...
 begin  
    ...
    from spMstItemPrice a, '+ dbo.GetDbMD(@CompanyCode, @BranchCode) +'..spMstItems b, 
	...
	begin
		...
		if (@intCountTemp > 0 ) begin 
			...
			if (@intStringEmpty < 1) begin
				set @QueryTemp = '
					insert into ' + dbo.GetDbMD(@CompanyCode, @BranchCode) + '..svSDMovement (CompanyCode, BranchCode, DocNo, DocDate, PartNo, PartSeq, WarehouseCode, QtyOrder, Qty, DiscPct, CostPrice, RetailPrice,   
 ...
 if (@DealerCode = 'SD' and @IsSPK = '2')  
 begin    
	set @QueryTemp = 'update ' + dbo.GetDbMD(@CompanyCode, @BranchCode) + '..svSDMovement   
