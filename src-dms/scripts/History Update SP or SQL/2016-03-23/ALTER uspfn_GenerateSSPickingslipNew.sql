...
(
	...
	FROM
		...
		LEFT JOIN #Item Item ON Item.CompanyCode = a.CompanyCode 
			...
			AND (a.SupplySlipNo IS NULL OR a.SupplySlipNo = '') -- Perubahan by RUDI - 2016-02-09
	...
	WHERE
		...
		and (a.DemandQty - a.SupplyQty) > 0 -- update
) a
GROUP BY
	a.CompanyCode
	, a.BranchCode
...
select top 10 * from spTrnSORDDtl where DocNO = ''SSS/16/001166'' and PartNo = ''12761M84430-000'' order by createddate desc
--===============================================================================================================================
-- INSERT SO SUPPLY
-- ==============================================================================================================================
...
begin
 ...
 declare @QueryTemp as varchar(max)  
	...
	' and a.CompanyCode =''' + @CompanyCode + '''' + -- Perubahan by RUDI - 2016-01-05
	' and a.BranchCode = ''' + @BranchCode + ''''; -- Perubahan by RUDI - 2016-01-05

	exec (@Query)
	--print (@QUERY)

	--set @Query ='select * from '+ @DbMD +'..svSDMovement where CompanyCode =''' + @CompanyCode + ''''
	--+ ' and BranchCode = ''' + @BranchCode + ''''
	--+ ' and DocNo = ''' + @TempDocNo + ''''
	--exec (@Query)
	--print (@QUERY)
