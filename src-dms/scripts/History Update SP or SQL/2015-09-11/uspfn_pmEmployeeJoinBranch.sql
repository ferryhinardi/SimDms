...
-- CREATED BY Benedict 18-May-2015 LAST UPDATE 11-Sep-2015
...
BEGIN TRY
	BEGIN TRANSACTION
		...
		IF (@KDPBranchCount > 1) RAISERROR(N'Ada konflik data KDP pada emp_id %s, harap hubungi IT Support', 15, -1, @EmployeeID)
		...
		BEGIN
			...
			BEGIN
				...
				UPDATE pmActivities
				SET BranchCode = @LeaderBranch, LastUpdateBy = @UserID, LastUpdateDate = GETDATE()
				...
				SET BranchCode = @LeaderBranch, UpdateUser = @UserID, UpdateDate = GETDATE()
