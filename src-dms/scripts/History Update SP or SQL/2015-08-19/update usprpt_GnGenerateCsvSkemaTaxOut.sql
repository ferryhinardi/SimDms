...
--select @CompanyCode=N'6114201',@BranchCodeFrom=N'',@BranchCodeTo=N'',@ProfitCenterCode='',@PeriodFrom='20150701',@PeriodTo='20150731',@ProductType=N'2W', @table='1', 
--@nomorfaktur = '0021554441184' 
...
having ROUND(SUM(sisaDPP),0) > 0) b
...
FROM @TblTempDetail a
GROUP BY a.CompanyCode, a.BranchCode, a.NOMOR_FAKTUR, a.InvoiceNo
having ROUND(SUM(sisaPPN),0) > 0) b
ON a.CompanyCode = b.CompanyCode
AND a.BranchCode = b.BranchCode
AND a.NOMOR_FAKTUR = b.NOMOR_FAKTUR
...
(SELECT TOP 1 KODE_OBJEK FROM @TblTempDetail WHERE CompanyCode = b.CompanyCode and BranchCode = b.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR 
and InvoiceNo = (SELECT TOP 1 InvoiceNo FROM @TblTempDetail WHERE CompanyCode = a1.CompanyCode and BranchCode = a1.BranchCode and NOMOR_FAKTUR = a1.NOMOR_FAKTUR ORDER BY PPN)) KodeTempDPP, 
...
FROM @TblTempDetail a
GROUP BY a.CompanyCode, a.BranchCode, NOMOR_FAKTUR, ProfitCenter) a1
left join 
...
and a1.ProfitCenter = '200'
and a1.cBranch > 1
and (case when b.TotalDPPAmt > a1.JUMLAH_DPP then b.TotalDPPAmt-a1.JUMLAH_DPP else a1.JUMLAH_DPP - b.TotalDPPAmt end) > 0
) b ON
...
GROUP BY a.CompanyCode, a.BranchCode, NOMOR_FAKTUR, ProfitCenter) a1
left join 
	(select a.CompanyCode, a.BranchCode, substring(REPLACE(REPLACE(TaxNo, '.', ''), '-', ''),4,13) TaxNo, FLOOR(SUM(DPPAmt)*0.1) TotalPPNAmt
	from gnTaxOut a
	where 
	a.CompanyCode = @CompanyCode
	and (CASE WHEN @BranchCodeFrom = '' THEN '' ELSE a.BranchCode END) BETWEEN @BranchCodeFrom AND @BranchCodeTo
...
SELECT DISTINCT
	...
	(SELECT Address1 + ' ' + Address2 + ' ' + Address3 FROM gnMstCustomer a WHERE a.CompanyCode = gnTaxOut.CompanyCode AND a.CustomerCode = gnTaxOut.CustomerCode)	
	  END END ALAMAT_LENGKAP
	, JUMLAH_DPP = (SELECT SUM(DPP) FROM @TblTempDetail WHERE CompanyCode = gnTaxOut.CompanyCode AND BranchCode = gnTaxOut.BranchCode AND NOMOR_FAKTUR = substring(REPLACE(REPLACE(gnTaxOut.TaxNo, '.', ''), '-', ''),4,13))
	, JUMLAH_PPN = (SELECT SUM(PPN) FROM @TblTempDetail WHERE CompanyCode = gnTaxOut.CompanyCode AND BranchCode = gnTaxOut.BranchCode AND NOMOR_FAKTUR = substring(REPLACE(REPLACE(gnTaxOut.TaxNo, '.', ''), '-', ''),4,13))
...
FROM 
	gnTaxOut 
INNER JOIN 
(SELECT CompanyCode, BranchCode, TaxNo, COUNT(a.TaxNo) cBranch
FROM gnTaxOut a 
GROUP BY CompanyCode, BranchCode, TaxNo) a ON 
	a.CompanyCode = gnTaxOut.CompanyCode AND a.BranchCode = gnTaxOut.BranchCode AND a.TaxNo = gnTaxOut.TaxNo
WHERE
