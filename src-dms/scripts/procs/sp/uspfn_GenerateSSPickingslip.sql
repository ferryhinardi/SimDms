alter procedure [dbo].[uspfn_GenerateSSPickingslip]      
...
--set @JobOrderNo  = 'SPK/13/012274'      
--set @ProductType = '4W'      
--set @CustomerCode = '2088486'      
--set @TransType  = '20'      
--set @UserID   = 'sa'      
--set @DocDate  = getdate()      
...
--select * from bitjkt..gnMstCompanyMapping    
...
declare @tmpSpMstItemLoc as table
(
...
declare @sqlSpMstItemLoc varchar(max)
--insert INTO @tmpSpMstitems
set @sqlSpMstItemLoc = 'select a.CompanyCode, a.BranchCode, a.PartNo, LocationCode, WarehouseCode, Onhand - AllocationSP - AllocationSL - 
AllocationSR - ReservedSP - ReservedSL - ReservedSR as QtyAvail 
from '
if @DealerCode = 'SD'
begin
	set @sqlSpMstItemLoc = @sqlSpMstItemLoc + @DbName + '..SpMstitemLoc '
end
else 
begin
	set @sqlSpMstItemLoc = @sqlSpMstItemLoc +  'SpMstitemLoc '
end
set @sqlSpMstItemLoc = @sqlSpMstItemLoc +  ' a JOIN #srvorder b on a.CompanyCode= b.CompanyCode and a.BranchCode= b.BranchCode and a.PartNo= b.PartNo'

--select a.CompanyCode, a.BranchCode, a.PartNo, LocationCode, WarehouseCode, Onhand - AllocationSP - AllocationSL - 
--AllocationSR - ReservedSP - ReservedSL - ReservedSR as QtyAvail 
--from SpMstitemLoc a JOIN #srvorder b
--on a.CompanyCode= b.CompanyCode
--and a.BranchCode= b.BranchCode
--and a.PartNo= b.PartNo
--print @sqlSpMstItemLoc
insert INTO @tmpSpMstItemLoc
EXEC (@sqlSpMstItemLoc)
select * into #tmpMstItemLoc from @tmpSpMstItemLoc
...
declare @DocYearMaxDocNo int
set @DocYearMaxDocNo = ISNULL((SELECT DocumentYear FROM GnMstDocument WHERE       
 CompanyCode = @CompanyCode      
  AND BranchCode = @BranchCode      
  AND DocumentType = 'SSS'       
  AND ProfitCenterCode = '300'       
...
SET @TempDocNo = ISNULL((SELECT 'SSS/' + RIGHT(@DocYearMaxDocNo,2) +'/' + RIGHT(1000000 + CONVERT(VARCHAR, @MaxDocNo, 1), 6)),'SSS/YY/XXXXXX')      
...
SELECT      
 ...
 , (SELECT LocationCode FROM @tmpSpMstItemLoc WHERE CompanyCode = a.CompanyCode AND BranchCode = a.BranchCode AND WarehouseCode = '00'      
  AND PartNo = a.PartNo) LocationCode      
 , a.QtyOrder      
 , CASE WHEN       
  ISNULL((SELECT QtyAvail      
   FROM @tmpSpMstItemLoc   WHERE CompanyCode = @CompanyCode AND BranchCode =@BranchCode AND WarehouseCode = '00'       
   AND PartNo = a.PartNo),0) >= a.QtyOrder       
  THEN a.QtyOrder       
  ELSE ISNULL((SELECT QtyAvail      
   FROM @tmpSpMstItemLoc   WHERE CompanyCode = @CompanyCode AND BranchCode =@BranchCode AND WarehouseCode = '00'       
 ...
 , CASE WHEN       
  ISNULL((SELECT QtyAvail      
   ...
   AND PartNo = a.PartNo),0) >= a.QtyOrder       
  THEN 0       
  ELSE a.QtyOrder - ISNULL((SELECT QtyAvail      
   FROM @tmpSpMstItemLoc   WHERE CompanyCode = @CompanyCode AND BranchCode =@BranchCode AND WarehouseCode = '00'       
   AND PartNo = a.PartNo),0)      
  END AS QtyBO      
 , 0 QtyBOSupply      
 ...
 , CASE WHEN       
  ...
  ELSE floor((ISNULL((SELECT QtyAvail      
    FROM @tmpSpMstItemLoc   WHERE CompanyCode = @CompanyCode AND BranchCode =@BranchCode AND WarehouseCode = '00'       
...
declare @sqlUpdtItemLoc varchar(max)
--insert INTO @tmpSpMstitems
set @sqlUpdtItemLoc = 'UPDATE '
if @DealerCode = 'SD'
begin
	set @sqlUpdtItemLoc = @sqlUpdtItemLoc + @DbName + '..SpMstItemLoc '
end
else 
begin
	set @sqlUpdtItemLoc = @sqlUpdtItemLoc +  'SpMstItemLoc '
end
   
SET @sqlUpdtItemLoc = @sqlUpdtItemLoc +' set AllocationSR = AllocationSR + b.QtySupply      
 , LastUpdateBy = '''+@UserID+'''      
 , LastUpdateDate = GetDate()      
FROM #tmpMstItemLoc a, #TempSOSupply b      
WHERE       
 1 = 1      
 AND a.CompanyCode = b.CompanyCode       
 AND a.BranchCode = b.BranchCode      
...
declare @DocYearMaxPickingList int
set @DocYearMaxPickingList = ISNULL((SELECT DocumentYear FROM GnMstDocument WHERE       
 CompanyCode = @CompanyCode      
  AND BranchCode = @BranchCode      
  AND DocumentType = 'PLS'       
  AND ProfitCenterCode = '300'       
...
SET @TempPickingList = ISNULL((SELECT 'PLS/' + RIGHT(@DocYearMaxPickingList,2) +'/' + RIGHT(1000000 + CONVERT(VARCHAR, @MaxPickingList, 1), 6)),'PLS/YY/XXXXXX')      
