...
go
if object_id('uspfn_HrInqEmployee') is not null
	drop procedure uspfn_HrInqEmployee

go
create procedure uspfn_HrInqEmployee
--declare
...
select a.CompanyCode
     ...
     , SubOrdinates = (select count(*) from HrEmployee where CompanyCode = a.CompanyCode and TeamLeader = a.EmployeeID  and PersonnelStatus=(case @Status when '' then a.PersonnelStatus else @Status end))
 ...
 where a.CompanyCode = @CompanyCode
   and a.Department like ( 
			case 
				when @DeptCode = '' or @DeptCode = '--' then '%'
				else @DeptCode
			end
       )
 ...
 order by a.CompanyCode, LastBranchName, a.Department, b.PosLevel desc, a.EmployeeName
